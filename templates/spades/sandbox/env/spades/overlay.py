...
def extract_overlay(env: Any) -> dict[str, Any]:
    ...
    draw: per-seat ``bids`` (``-1`` until a seat has bid) and ``tricks_won``, and three views of the
    score for two surfaces. The two-element ``team_scores`` is what the local pygame renderer and the
    ...
    ``legal_actions`` is the phase-legal action set for the seat on turn, empty once the hand is
    terminal, and it is what both renderers grey from.
    """
    state = env.state
