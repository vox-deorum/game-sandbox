...
from sandbox.env import AUTO_ACTION, make_env
from sandbox.env.spades import rules


def test_encoding_matches_the_rules_engine():
    # The helpers restate the fixed card and bid encoding; the rules engine is its source of truth.
    # If a future change touched one and not the other, this catches it for every card and bid.
    ...
    for bid in range(NUM_BIDS):
        assert bid_to_action(bid) == rules.bid_to_action(bid)
        assert action_to_bid(bid_to_action(bid)) == bid
    ...
    try:
        ...
        while env.agents:
            ...
            if rules.in_bidding(state):
                ...
                assert legal_cards(observation) == rules.legal_actions(state, seat)
                assert legal_bids(observation) == []

            # Scalar / list state accessors unwrap the arrays to plain Python values.
            ...
            # The live trick reproduces the rules' play-order pairs, and the winner-so-far and the
            # would-this-card-win predicate agree with rules.trick_winner over the same cards.
            played = current_trick(observation)
            assert played == [(s, c) for s, c in state.current_trick]
            if played:
                ...
                if not rules.in_bidding(state):
                    for card in legal_cards(observation):
                        expected = rules.trick_winner([*state.current_trick, (seat, card)]) == seat
                        assert beats_current_winner(observation, card) is expected
            else:
                assert trick_winner_so_far(observation) is None

            # The completed-trick readers mirror the recorded last trick (seat -> card) and winner.
            if state.last_trick is None:
                assert last_trick(observation) == []
                assert last_trick_winner(observation) is None
            else:
                assert dict(last_trick(observation)) == {s: c for s, c in state.last_trick}
