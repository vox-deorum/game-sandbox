...
queen flips your score to 0 and gives everyone else 26.
"""
...
class Agent:
    ...
    - ``observation["action_mask"]`` is a length-52 array; ``mask[c] == 1`` exactly for the
      cards you may legally play right now. **You must return a card whose mask bit is set** —
      anything else is rejected. The mask already encodes every rule (follow suit, hearts not
      led until broken, the first-trick restrictions), so you never re-derive legality.
    - ``observation["observation"]`` holds the table state: ``hand`` (length-52, 1 where you
      hold the card), ``trick`` (length-4, the card each seat has played this trick or ``-1``),
      ``led_suit`` (``0..3`` or ``-1`` when you are leading), ``hearts_broken`` (0/1),
      ``position`` (your seat ``0..3``), ``trick_leader``, and ``scores`` (running penalty
      points per seat).
    """
