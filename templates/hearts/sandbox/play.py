"""Play one game of Hearts locally, against vanilla PettingZoo.

    python -m sandbox.play               # watch YOUR agent play a seat against the built-in bots
    python -m sandbox.play --headless    # no window: play it out and print the final scores
    python -m sandbox.play --human       # take a seat yourself and click your legal cards
    python -m sandbox.play --seat 2      # sit in a different seat (0..3)
    python -m sandbox.play --seed 7      # pick the deal

Hearts is turn-based with four seats, so it does not fit the single-slot realtime loop the base
template ships; this env-layer file overrides ``sandbox/play.py`` whole-file. It keeps the same
surface the provided tooling reads — ``load_agent``, ``play_episode``, and a ``main`` driving the
same observation/action cycle the server runs — but seats your one agent (or you) among three
built-in opponents and steps the table a turn at a time through the provided ``sandbox.env``.

The built-in opponent is the environment's own timeout default: it plays its lowest legal card.
It is a fine, low-risk baseline to measure your agent against; clearing it is the example's bar.

When there is a window the game begins on a manual interaction (any key or click) and holds the
final scores until you dismiss them; ``--headless`` opens no window — it (and ``evaluate`` and the
tests) just plays the deal out and reports the result. This loop is the contract: the server wraps
this exact stepping with timeouts, recording, and the move clock for the human seat.
"""
...
import pygame
from sandbox.env import PLAYER_SLOT, default_action, make_env, make_human_controller
...
def play_episode(
    ...
    *,
    ...
    Returns ``PLAYER_SLOT``'s cumulative reward — for Hearts that is its final leaderboard score
    (higher is better; the negated penalty total, so ``0`` is a clean game and ``-26`` is a
    disastrous one). The shared stepping contract: headless ``evaluate`` and the inherited tests
    call this with no ``on_frame``, while the windowed run passes one to draw each frame.
    ...
    while env.agents:
        ...
        if agent_id == PLAYER_SLOT:
            ...
            # Built-in opponent: the environment's own timeout default — its lowest legal card.
...
def play_table(
    ...
    *,
    ...
    Turn-based: on your turn the human controller blocks until you click a legal card; every other
    move lingers a beat so the table is followable. The window begins paused on the first frame and
    holds the final scores until dismissed. Closing the window stops the game and returns early.
    """
    env.reset(seed=seed)
    # Hearts shows the chosen seat at the bottom of the table and fans its hand for clicking.
...
def _hold_final_frame(env: Any, timeout_s: float = 8.0) -> None:
    ...
    terminal frame with the final per-seat scores on the last step; we just keep it live.
    """
    if not pygame.display.get_init():
        return
...
def _report(seat: int, scores: dict[str, float]) -> None:
    """Print the final per-seat result. Cumulative reward is the leaderboard score (-penalty)."""
    if not scores:
        print("no result (the window was closed early)")
        return
    penalties = {slot: -score for slot, score in sorted(scores.items())}
    line = "   ".join(f"{slot}: {int(points)}" for slot, points in penalties.items())
    print(f"final penalties (lower is better):   {line}")
    your_slot = f"player_{seat}"
    if your_slot in penalties:
        print(f"you played {your_slot} -> {int(penalties[your_slot])} penalty points")
...
def main(argv: list[str] | None = None) -> int:
    ...
    enable_hidpi()
    parser = argparse.ArgumentParser(description="Play one game of Hearts locally.")
    ...
    parser.add_argument("--human", action="store_true", help="play a seat yourself by clicking cards")
    ...
    if args.human:
        ...
        # clicks against the hand it draws.
        ...
        try:
            ...
            env.close()
        _report(args.seat, scores)
    elif args.headless:
        # No window, but still drive the renderer (rgb_array) each step so the local play path —
        # loop plus renderer — is exercised end to end, then report the final score.
        agent = load_agent(REPO_ROOT)
        env = make_env(render_mode="rgb_array")
        try:
            score = play_episode(
                agent, env, seed=args.seed, max_steps=args.steps, on_frame=lambda: env.render()
            )
        finally:
            env.close()
        print(f"seed {args.seed}: your leaderboard score {score:.2f} ({int(-score)} penalty points)")
    else:
        agent = load_agent(REPO_ROOT)
        env = make_env(render_mode="human")
        try:
