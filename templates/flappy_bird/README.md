...
This is the starter kit for writing a Game Sandbox agent. You develop and test entirely on your own machine against vanilla PettingZoo, with no backend, account, or network connection. When you are ready, submit the repository through GitHub. The template already has the required shape: fill in `agent.py`, keep the manifest and pinned dependency set, and you are done.
