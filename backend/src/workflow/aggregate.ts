 ...
 * produced and sums the per-tick fields. For one player this yields three numbers the `game_results`
...
/**
 * Aggregate one player's contribution across a recording's per-step states. The final score is the
 * last state that reported the player's `score`; compute time and tick count come from the ticks that
 * carry the player's `timing.decision_ms`, with `learn_ms` and `chat_ms` folded in (zero when absent).
 */
export function aggregateSeat(states: readonly StepState[], playerId: string): SeatAggregate {
