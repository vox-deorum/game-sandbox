 ...
 *
 * Compute time is `decision_ms + learn_ms` summed over the slot's timing-bearing ticks. `learn_ms` is
 * present only for learning agents (the Naive baseline emits no learn hook), so an absent value is
 * treated as zero. There is no single recorded "acted tick count" field; it is derived as the count
 * of ticks that carry this slot's timing. When chat and LLM-wait timing land in later stages they are
 * added to the same per-tick total here, and the runner keeps only aggregating.
 */
...
export interface SeatAggregate {
  ...
  /** Sum of `decision_ms + learn_ms` over the seat's timing-bearing ticks. */
 ...
 * the slot's `timing.decision_ms`, with `learn_ms` folded in (zero when absent).
 */
export function aggregateSeat(states: readonly StepState[], slotId: string): SeatAggregate {
  ...
  for (const state of states) {
    ...
    if (timing !== undefined && typeof timing.decision_ms === 'number') {
      agentComputeMsTotal += timing.decision_ms + (timing.learn_ms ?? 0)
      actedTickCount += 1
    }
  }
