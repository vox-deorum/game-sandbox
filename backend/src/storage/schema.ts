...
/**
 * The `session_submissions` table: which submitted agent ran in which session seat. Keyed by
 * `(session_id, seat_id)`, the seat within a session is the natural key, so there is no surrogate
 * id. Stage 5 writes one row per submitted-agent watch session (`player_0` for Flappy Bird); Stage
 * 8 extends it to one row per submitted seat in multi-agent sessions.
 */
