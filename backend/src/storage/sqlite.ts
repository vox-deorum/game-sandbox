...
/**
 * Open (creating if needed) the SQLite database at `dbPath`, migrate it to the latest schema, and
 * return a ready {@link Storage}. Pass `:memory:` for an ephemeral database (the test default). The
 * migration is idempotent, so reopening an existing file is safe. The implementation is the real one
 * in every context, per the storage design.
 */
