 ...
 * trim, the active-`ready` exemption, and debris tolerance — the four properties the real Docker
 * `listOverlayImages`/`removeImage` then ride in the Docker-gated suite.
 */
...
function image(submissionId: string, createdAtMs: number): OverlayImage {
  return { ref: `overlay:${submissionId}`, submissionId, createdAtMs }
}

const BIG_BUDGET = { overlayImageBudget: 50, overlayImageSweepIntervalMs: 3_600_000 }
...
describe('overlay eviction — debris and failure tolerance', () => {
  ...
  it('continues past a remove failure and does not throw', async () => {
    ...
    const eviction = new OverlayEviction(
      driver,
      readyReader([]),
      { overlayImageBudget: 0, overlayImageSweepIntervalMs: 3_600_000 },
