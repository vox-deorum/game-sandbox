...
describe('submission storage on :memory:', () => {
  ...
  async function iterationId(envId = 'flappy_bird'): Promise<string> {
    const iter = await storage.ensureOpenIteration(envId, 1)
    return iter.id
  }

  it('ensureOpenIteration is idempotent for an environment', async () => {
  ...
  it('createSubmission inserts a pending row and supersedes a same-user resubmit', async () => {
    ...
    const first = await storage.createSubmission(
      subInput({ iteration_id: iter, created_at: '2026-06-11T00:00:00.000Z' }),
    )
    ...
    const second = await storage.createSubmission(
      subInput({ iteration_id: iter, created_at: '2026-06-11T00:05:00.000Z' }),
    )
  ...
  it('does not supersede a different user in the same iteration', async () => {
    const iter = await iterationId()
    const alice = await storage.createSubmission(subInput({ iteration_id: iter, user_id: 'alice' }))
    const bob = await storage.createSubmission(subInput({ iteration_id: iter, user_id: 'bob' }))
  ...
  it('keeps exactly one active row when two submits race for the same participant', async () => {
    ...
    await Promise.all([
      ...
      storage.createSubmission(
        subInput({ iteration_id: iter, created_at: '2026-06-11T00:00:01.000Z' }),
      ),
    ])
    const active = await storage.listActiveSubmissionsByIteration(iter)
  ...
  it('updateSubmissionPin records the commit for git and leaves local commitless', async () => {
    const iter = await iterationId()
    const git = await storage.createSubmission(subInput({ iteration_id: iter, user_id: 'alice' }))
  ...
  it('updateSubmissionStatus records failure reasons and clears reason on ready', async () => {
    const iter = await iterationId()
    const sub = await storage.createSubmission(subInput({ iteration_id: iter }))
  ...
  it('listPendingSubmissions returns active pending rows newest-first and skips superseded', async () => {
    ...
    const older = await storage.createSubmission(
      subInput({ iteration_id: iter, user_id: 'alice', created_at: '2026-06-11T00:00:00.000Z' }),
    )
    // Supersede alice's older submission with a newer one.
    const newer = await storage.createSubmission(
      subInput({ iteration_id: iter, user_id: 'alice', created_at: '2026-06-11T00:09:00.000Z' }),
    )
    const bob = await storage.createSubmission(
      subInput({ iteration_id: iter, user_id: 'bob', created_at: '2026-06-11T00:03:00.000Z' }),
    )
  ...
  it('listActiveSubmissionsByIteration filters superseded and narrows by status', async () => {
    const iter = await iterationId()
    const alice = await storage.createSubmission(subInput({ iteration_id: iter, user_id: 'alice' }))
    ...
    const all = await storage.listActiveSubmissionsByIteration(iter)
    expect(all.map((s) => s.id).sort()).toEqual([alice.id, bob.id].sort())
    const ready = await storage.listActiveSubmissionsByIteration(iter, 'ready')
    expect(ready.map((s) => s.id)).toEqual([alice.id])
  })

  it('listActiveReadySubmissionIds returns only active ready ids across iterations', async () => {
    const flappyIter = await iterationId('flappy_bird')
    const otherIter = await iterationId('turn_based')

    const ready = await storage.createSubmission(
      subInput({ iteration_id: flappyIter, user_id: 'alice' }),
    )
    await storage.updateSubmissionStatus(ready.id, 'ready')
    // A ready submission in another iteration is also included.
    const otherReady = await storage.createSubmission(
      subInput({ iteration_id: otherIter, env_id: 'turn_based', user_id: 'carol' }),
    )
    ...
    const failed = await storage.createSubmission(
      subInput({ iteration_id: flappyIter, user_id: 'bob' }),
    )
