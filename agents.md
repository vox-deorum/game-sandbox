...
- After completing a piece of UI work, run the browser end-to-end suite (it needs a running Docker daemon). The default `check` and `test` loop only lints, typechecks, and runs the jsdom unit tests, none of which exercise the live DOM or its locators, so a renamed class or restructured markup can pass everything local and still break the Playwright journeys that CI runs. The e2e suite is the only thing that catches that before CI does.
  - While iterating, run only the group that covers your change: `uv run python scripts/ci.py frontend-e2e --group <name>`, repeating `--group` for several. The groups are `auth`, `play`, `seasons`, `submissions`, `local`, `hearts`, `spades`, and `crane-reach`, one per directory under `frontend/e2e/`. Use `--fast` for every group without the long season arcs, and add `--no-build` to reuse the bundles from the previous run.
