...
The site serves two audiences with different needs: students using the template to write agents, and developers of this repository. MkDocs Material's `navigation.tabs` gives each a top-level tab, and the specification gets a third tab of its own (see below):
...
At the end of Stage 1 the home page and the five contributor pages are real. The student pages exist as honest stubs that name the stage delivering their content, so the navigation shape is stable from day one and links from the placeholder template README never break.

## The specification tab

The specification under `specs/` is rendered on the site as its own top-level tab, alongside the student and contributor tabs. The files are not duplicated: `specs/` remains the only editable source, and a small MkDocs hook (a Python file referenced from `mkdocs.yml` through the `hooks` option) injects every `specs/*.md` file into the site under `specs/` at build time. The hook keeps `mkdocs build` and `mkdocs serve` self-contained, with no copy step to forget. Because the spec files cross-link each other with plain relative links, they render unchanged as a flat section, and the strict build verifies every link inside them just as it does for docs pages.

Division of labor between the tabs: spec pages describe what the system is, docs pages are operational guides for how to set up, how the contract behaves in practice, and how publishing works. Where a docs page touches design, it links to the rendered spec page rather than restating it. The one normative exception is `contributors/state-schema.md`, which is the public home of the version rule and the sidecar rule from [state-schema.md](state-schema.md); the schema files carry descriptions and `schema/README.md` is a pointer to the docs page.
