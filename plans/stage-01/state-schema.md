...
This wording is normative and is mirrored on the docs site. A sidecar is an auxiliary file stored alongside a recording, declared in the header's `sidecars` array by `name` and `path`. The `name` identifies the sidecar's kind against a registry of known names, which is empty. The `path` is relative to the recording's own directory. A reader that does not recognize a sidecar name must skip that entry and load the recording normally. Unknown keys inside a sidecar entry are likewise ignored. Any sidecar payload schema uses the same `schema_version` as the recording header, so adding one is an additive contract change, never a second storage format.
