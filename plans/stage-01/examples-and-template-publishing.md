...
`.github/workflows/template-publish.yml` triggers on pushed tags matching `template-v*` and runs three jobs in order. First, verify: compose and test all examples at the tagged commit, so a broken state cannot publish. Second, publish the template content to the student repository's main branch with the mirrored tag. Third, compose and push each example to its `examples/<name>` branch. Authentication uses a fine-grained personal access token with contents write on the target repository, stored as the `TEMPLATE_REPO_TOKEN` secret; a GitHub App is the documented upgrade if token rotation becomes a burden. Marking the target as a GitHub template repository is a one-time manual setting.

## What "proven" means in Stage 1

Stage 1 ships a placeholder template: a README with a visible warning that the real template arrives in Stage 2, a pinned `requirements.txt` with at least one real dependency, a `requirements-dev.txt`, a trivial module, and a passing `tests/`. Alongside it, `examples/hello/` overrides one file and adds one extra pinned dependency. The machinery is proven when pushing `template-v0` updates the student repository's main branch, its `v0` tag, and the `examples/hello` branch end to end; when regular CI composes and tests the example on every PR; and when a unit test asserts the conflicting-pin failure mode.
