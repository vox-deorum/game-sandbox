...
Every environment explicitly declares a `PUBLISHED_EXAMPLES` tuple. It selects the source examples that may be published, and may be empty. The declaration names distinct immediate `examples/<name>` directories and is validated with the complete generated branch name before publication. This selection does not change source discovery: all checked-in examples are composed and tested by examples CI.
