...
A new environment package under `environments/src/game_sandbox_environments/hearts/`, registered as a `game_sandbox.environments` entry point in `environments/pyproject.toml`, mirroring `flappy_bird/__init__.py`. It exposes the standard `EnvironmentEntry` (meta, `make`, `default_action`, optional `overlay`).
