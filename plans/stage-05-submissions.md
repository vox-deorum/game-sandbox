...
4. **[Sandboxed load check and overlay build](stage-05/4-load-check-and-overlay-build.md).** The harness `validate` command and a `submission-overlay` image spec on the driver; the load check runs against the built overlay image under the no-network, read-only profile with a short timeout.
...
The validation log is modeled as a dedicated, append-only `submission_checks` table (one row per pipeline stage) rather than a JSON column on the submission, keeping the relational schema the single source of truth per [stage-03/backend-skeleton-and-storage.md](../stage-03/backend-skeleton-and-storage.md) and giving the agent profile ordered, queryable history. The log records the pipeline's structured stage outcomes and the captured rejection reason or error text; streaming raw build/container output line-by-line is out of scope for this stage and can be layered onto the same table later without changing the poll contract.
