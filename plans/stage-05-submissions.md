...
Extend the session orchestrator so a session can name submissions for its non-human slots and run the corresponding image. Wire the watch self-play flow for Flappy Bird (pick an agent, run it in the single slot, stream to the renderer). Keep human Flappy Bird play as a human-controlled single-slot session; choosing submitted opponents for a human appears when an environment actually exposes both human and agent slots, with the first full multi-agent version in Stage 9.

Build the agent profile page: submission history across iterations, recent replays, and placeholders for leaderboard placements (Stage 6) and the LLM debug view (Stage 8).
