...
`deriveStatus` is a pure function of `role` alone. The canonical singleton `admin` maps to `admin`, `user` maps to `normal`, and `pending` maps to `pending`. Better Auth can store multiple roles as a comma-separated string, but Game Sandbox deliberately does not: any composite value, unknown value, empty token, or missing role maps to `pending`, which fails active and admin guards closed. Thus both `admin,user` and `pending,user` are pending, and an unrecognized value never becomes active. The direct admin-plugin boundary applies the same singleton-admin requirement in [step 1](1-better-auth-foundation.md), so plugin and application authorization agree even for a corrupted row. Ban is not part of this mapping: it is a standalone flag Better Auth enforces by revoking sessions and blocking sign-in, so a banned user never carries a live session and `deriveStatus` is only ever reached for a non-banned user. Status is therefore one of three values; `banned` is not a request-time status.
