...
`deriveStatus` is a pure function of `role` alone. Better Auth stores `role` as a string that may hold a comma-separated list, so `deriveStatus` splits it and applies a fixed precedence: if any token is `admin` the status is `admin`, else if any token is `user` the status is `normal`, else `pending`. An empty token, an unknown value, and a missing role all contribute nothing and fall through to `pending`, which fails the active and admin guards closed. Thus `admin,user` derives `admin` and `pending,user` derives `normal`, matching exactly how the admin plugin comma-splits the same string for its own permission checks in [step 1](1-better-auth-foundation.md), so plugin and application authorization agree even for a hand-corrupted row. A supported operation only ever writes a single scalar role, so these composite cases arise only from direct database tampering. Ban is not part of this mapping: it is a standalone flag Better Auth enforces by revoking sessions and blocking sign-in, so a banned user never carries a live session and `deriveStatus` is only ever reached for a non-banned user. Status is therefore one of three values; `banned` is not a request-time status.
...
| `POST` and `DELETE /api/recordings/:id/pin`, `GET /api/submissions`, `GET /api/submissions/:id`, `GET /api/sessions/:sessionId/ratings`, `GET /api/seasons/:seasonId/agent-rating-prompt` | `requireUser`. A pending user may look at their own things, including prior personalized rating state and their own saved author prompt; the existing ownership checks are unchanged, and an admin keeps the operator override on `GET /api/submissions/:id`. Anonymous callers receive `401 auth_required` rather than a view personalized to a fabricated or empty identity. |
...
Rewrite `backend/test/identity.test.ts` around the new surface: `deriveStatus` covers the scalar roles and proves the comma-split precedence (`admin,user` maps to `admin`, `pending,user` maps to `normal`) while missing roles, empty tokens, and unknown values all map to pending; `resolveUser` treats a banned user as anonymous; and guard behavior over a stub `Auth` proves an anonymous request gets `401 auth_required`, a pending user gets `403 not_active` from `requireActive`, and a non-admin gets `403 not_operator` from `requireAdmin`.
...
- The matrix holds: a pending user gets `403 not_active` on session start, submit, reachability, a rating write, and an author-prompt write; an anonymous request gets `401 auth_required` on those routes and on both personalized rating reads; pin, unpin, own-submission reads, a prior rating view, and an own author-prompt read work for a pending user; and public reads work anonymously.
