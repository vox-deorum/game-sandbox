...
- `storage/index.ts` — the `Storage` interface, speaking the derived domain types: `createSession`, `markRunning`, `markEnded`, `findActiveSessionByUser`, `getSession`, `listSessions`. Callers see a domain-shaped API, never SQL or query building; this interface is the seam the orchestrator and HTTP tests are written against.
