...
`SandboxProfile` expresses the sandbox in driver-neutral terms, per the spec: `cpus`, `memoryMb`, `readOnlyRoot` (always true), `scratch` (`containerPath` plus `sizeMb`), `network` (`none` in this stage; the internal gateway-only value arrives in Stage 9), and `mounts` (host path, container path, read-only flag — the recordings volume is the one mount this stage uses). Each driver maps the profile onto its platform.
