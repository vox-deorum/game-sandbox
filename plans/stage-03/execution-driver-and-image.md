...
There is one base image per dependency-set version, tagged `game-sandbox/session-base:deps-v<N>`, per [execution.md](../../docs/specs/execution.md). This stage needs only v1. The Dockerfile lives at `backend/images/session-base/Dockerfile` and builds with the repo root as context, since the image is assembled from monorepo sources. It is `python:3.12-slim`, plus the pinned dependency set installed from `templates/base/requirements.txt` (the compiled v1 set from Stage 2), then the `harness` and `environments` packages installed from source with `--no-deps` so the pinned set stays exactly authoritative. Their own pins are kept compatible with the set, per Stage 2, but the set is the single source of dependency truth inside a session.
...
Several details were confirmed during the build (`backend/images/session-base/Dockerfile`):
...
- The built-in agent is composed with `scripts/compose.py` and copied to `/opt/agents/builtin`, and the example's extra pins (`wcwidth`) install on top so the agent loads. Extras extend the set, never override it.
