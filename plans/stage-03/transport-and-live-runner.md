...
Backend to browser uses the same split, per the parent file. Server-to-client frames are JSON text: the header and state lines relayed verbatim, the relayed `result` envelope, relayed `pause`/`resume` echoes so every attached client can show paused state, and one backend-originated envelope, `{"kind": "session", "status": "running" | "ended", "reason"?}`. On attach the backend sends the buffered header, the most recent state, and the current session status so a renderer can draw immediately; full catch-up and scrubbing belong to the Stage 4 replay viewer, not the live socket. Client-to-server frames are the same command envelope as the container side; the backend validates the envelope shape and that the sender controls the slot, then forwards — it never interprets actions, because the container is authoritative. Slot ids ride every input so later multi-human sessions are protocol-compatible without a new transport.

## Exposing the step machinery

Pacing has to apply to every live session — a watch session with only the built-in agent must still advance at the environment's cadence, so pacing cannot hide inside a human action source. `session.py` is therefore refactored to expose the machinery `run_episode` already contains: an `Episode` object owning the reset state, a `step_once()` that advances the acting slot (action acquisition, environment step, `learn`, state assembly, recording write, budget accounting), and the termination checks. `run_episode` becomes a thin loop over `Episode` with byte-identical behavior — the Stage 2 determinism fixtures must pass unchanged, which is the regression gate for the refactor. The live loop is a second thin loop over the same machinery: before each step it waits until the next cadence instant when the environment has a pace interval and does not wait when it has none, checks the pause and stop flags, then calls `step_once()`. That conditional on the pace interval is the entire realtime-versus-turn-based difference, which is the "one code path reading the pace interval" the parent file requires.
...
harness/src/game_sandbox_harness/
  live.py     python -m game_sandbox_harness.live: config parsing, stdout hygiene, the live loop
  live_io.py  the stdin command pump, latched inputs, PausableClock, TransportSource, the tee store
```

- **Session config** arrives as a single JSON argument in the container argv (via `LaunchSpec.args`): env id, seed, slot bindings (`external` or `builtin-agent` per slot), the resolved human-slot timeout, and the recording id and directory. Metadata defaults — pace interval, time limits, default human timeout — come from the registry inside the image, so the config carries only overrides and there is one source of truth for environment facts. Session start accepts the human-slot timeout override, defaults it from metadata, and the resolved value lands in the `ExternalSlot` timeout, satisfying the parent's override requirement. Stdin is reserved for runtime commands.
- **Stdout hygiene.** Protocol lines must own stdout, but pygame prints a banner on import. The first thing `live.py` does is `os.dup()` the real stdout for the protocol writer and `os.dup2()` stderr over fd 1, before any game import, so stray prints from environment internals land in the diagnostics stream instead of corrupting the protocol.
- **The command pump.** A reader thread consumes stdin lines and updates shared state: the latest input per slot (latching, per [interaction.md](../../specs/interaction.md)), the pause flag, the stop flag. The stepping thread never blocks on stdin.
...
- **The tee store** wraps `FolderRecordingStore`: each header and state line is serialized once and written both to the recording file on the mounted volume and to the protocol stdout, so the streamed bytes and the stored bytes cannot differ; a test asserts the parity byte-for-byte.
- **Built-in agent slots** load `/opt/agents/builtin` through the Stage 2 manifest loader — the same path Stage 5 submissions take.
- **Lifecycle.** On episode end (`terminated`, `truncated`, `episode_limit`) or the `stop` command (reason `stopped`), the runner closes the recording, emits the `result` envelope, flushes, and exits 0. Uncaught errors log to stderr and exit nonzero; the orchestrator records the failure. Per-step and per-episode budgets run unchanged inside the container; the wall-clock backstop against a hung container lives in the orchestrator, per [orchestrator-and-http-api.md](orchestrator-and-http-api.md).
