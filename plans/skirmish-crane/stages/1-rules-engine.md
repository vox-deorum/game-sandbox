...
| tile_types.json | The single tile-type source: terrain and feature properties (passability, move cost, feature move-cost delta, entry damage), the generation scatter tables, and the compact overlay wire codes. The renderer reads the same file directly, so the two sides cannot drift. |
| tile_types.py | The typed loader for tile_types.json, validating the document and exposing typed terrain, feature, scatter-table, and wire-code lookups to the rest of the engine |
...
Generation is constructive rather than check-and-reject: generate one half, point-reflect through (q, r) to (2R - q, 2R - r), and the symmetry guarantee holds by construction. Water passages are carved first so their count (2 or 3) and width (2 to 4) hold by construction. Every other passable tile on the half then draws two independent rolls from the tile_types.json scatter tables, a 20-sided roll for terrain (0 gives hill, otherwise grass) and a 20-sided roll for feature (0 forest, 1 marsh, 2 wasteland when the wasteland parameter is on, otherwise none), and mirrors the result. Terrain and feature are drawn independently, so a feature may land on any passable terrain, hills included, and each kind keeps its own density of about 5 percent of eligible tiles. Spawns mirror each other. Capture zones are seven-tile blocks (a passable center plus its six passable neighbors) placed as one central zone plus mirrored pairs. Chosen centers are kept at least 3 apart in hex distance, which is exactly the threshold that keeps two seven-tile footprints disjoint, so no unit can ever stand in two zones and score both in one round. Turning capture zones on forces three passages, because only the three-wide central gap keeps the middle tile passable and an odd zone count always needs a central zone there. Connectivity of passable tiles is verified by flood fill, with a bounded, seed-deterministic redraw loop when a draw fails.
...
- Entering a tile applies its feature's entry damage, once per tile entered along the path including the final tile, floored at 1 hit point so it never kills. This happens during the walk, before the strike, and is separate from the strike damage stack below.
...
- Wasteland entry damage: 2 damage per tile entered, the floor of 1 hit point, safety while standing still, paying twice for entering the same tile twice in one path, and generation only placing wasteland tiles when the wasteland parameter is on.
- The tile-type registry: tile_types.py loads and validates tile_types.json, every declared terrain and feature pairing has a wire code, and a malformed document is rejected.
