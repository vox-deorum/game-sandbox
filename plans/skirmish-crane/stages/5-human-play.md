...
Resolved events use three fog phases. The starting perspective remains active through movement. When movement or the stationary activation cue finishes, the renderer recomputes visibility from the same resolving observer set at final positions and installs it immediately before the strike or capture reaction, so an arriving archer can visibly acquire its target. A defeated event target may be retained only in the temporary presentation set for the hit and death animation, while unrelated units stay hidden. A terminal arriving state uses the existing full-board terminal policy. The next-activation perspective is installed only after the settled hold.
...
- Preview tests cover a unique nearest in-range enemy, tied nearest candidates marked uncertain, no in-range enemy, endpoint changes, and reset to the origin. Activation-seal tests cover the 1.6-second human-composition fade and the steady spectate, replay, and event seals. Step-number tests verify mono text is baked at host scale times camera zoom. Event timing tests cover the `CRANE_TIMING` 300 ms controlled-order hold, 200 ms watched-event hold, and no hold for snap or invisible updates. An activation resolved out of sight is proved to install without animating, and one the perspective could see is proved to play out before the fog follows the next unit. Clock tests cover the Confirm button's full perimeter draining clockwise, the ember threshold, a reconnect landing mid-turn, freezing under a playback pause, and fake-clock progression.
- Fog tests cover human turns, companion turns, opponent turns, a controlled unit's death while companions remain, terminal states, reconnect mounting, direct rendering, and the three resolved-event phases: starting visibility through movement, final-position visibility before the reaction, and next-activation visibility after the settled hold. They also verify that a defeated event target is retained only for its hit and death presentation, unrelated units remain hidden, and terminal arrivals use the full-board policy. Terrain stays complete.
