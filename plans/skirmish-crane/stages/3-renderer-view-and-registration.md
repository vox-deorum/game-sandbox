...
- Each dynamic overlay carries a roster-order base-64 bitmask `d` of completed activations in the current round. The scene reads it directly for unit `hasActed` state; older version 1 recordings without this field display no completion marks. Acted units use opaque muted artwork and a bone checkmark, with readable side colors and unchanged HP gauges. Their inspection card shows only a checkmark beside the name. The resolving actor receives this treatment after its animation finishes, and marks reset with the round. Direct replay seeks need no preceding frames.
