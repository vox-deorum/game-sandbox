...
- Layering for 40 units across 6000 ticks: a static battlefield layer built once per episode, a reconciled unit layer, and an event layer animating the overlay's most recent events, with snap semantics on seeks and durations scaled by the paced host's cadence, a minimum delivery interval rather than a budget. Compact overlay version 2 supplies each event's exact executed path id, which the event layer expands into its entered-tile route.
