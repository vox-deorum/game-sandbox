...
- Reset, reselect, or undo: a centered two-button pair occupies the lower strip, with Reset on the left and Confirm on the right. Reset uses `glyph-reset.png`, is always drawn, and is muted and inactive until at least one step is selected. Its accessible name and tooltip are `Reset movement`. When active, it clears the full selected path, restores full movement allowance, and recomputes the strike preview from the origin without submitting an action or restarting the move clock. A chosen enemy target persists through path edits and Reset. Clicking the last path tile still removes that step, clicking the activated unit still clears the path, and the reverted tile's highlight pulses once.
- Target selection and preview: clicking any visible living enemy selects its stable enemy-roster target value, and clicking it again restores automatic targeting. The selected target is marked with a drawn gilt ring, while hover inspection keeps its own temporary bone ring. Every target or endpoint change shows an informational ink thread from the projected final tile to the actual strike target, at the ranged arc's weight and dashed on a fixed rhythm so an adjacent strike reads the same as a distant one. An in-range selected target takes precedence. When movement puts a selected target out of range, the selection remains, its ring switches to the existing ember warning color, and a diagonal slash appears at the target; the preview continues to show the nearest in-range automatic strike, with tied nearest candidates each getting their own thread. Moving back into range restores the normal gilt marker. Automatic targeting uses the same nearest and tie rules. When no enemy is in range, there is no preview. The preview reads the enemies the person can see, sends nothing, and never advances the match. A new activation clears target selection. Spectators and replay viewers retain hover inspection but cannot select targets.
- Confirm: the right icon button uses `glyph-move.png`, with accessible name and tooltip `Confirm order`. It sends the selected path and target value, using `target: 0` for automatic targeting; with an empty path it sends the usual stand-still-and-strike order.
...
- Human-control coverage verifies the faded activation seal only during open composition, steady seals outside human composition, absent movement pips, target selection and its drawn gilt ring, the ember warning ring with diagonal slash when movement puts the selected target out of range, restoration of the normal marker when it returns to range, selection persistence through path edits and Reset, automatic restoration on a second click, clearing on a new activation, reset activation after a selected step, reset restoration of the empty path and movement allowance without an action or clock restart, and sharp step-number resolution after zooming.
