...
The helper package's `forecast.from_observation(observation)` constructs an independent, already initialized regular `SkirmishCraneEnv` from the observation alone. It uses the observed self as the next actor, visible `has_acted` values to skip the rest of the current round, a local shuffle for other visible units, and deterministic local randomness seeded at 0. It supports normal PettingZoo inspection, stepping, and dead-player cleanup. Importing helpers stays light; constructing a forecast loads the engine. The forecast contains no unseen units, so elimination, capture, and result values are local approximations and hidden units never emerge. It accepts no hidden-state estimate or extra seed, and provides no policy, pathfinding, or search.
...
During a resolved event, fog follows three presentation phases. The starting perspective remains active while the unit moves. When movement or the stationary activation cue finishes, the renderer recomputes visibility from the same resolving observer set at the units' final positions, then installs that perspective immediately before the strike or capture reaction begins. A target removed by the event may remain in the temporary presentation set only for its hit and death animation. Unrelated units remain hidden. If the arriving state is terminal, the existing full-board terminal policy applies. The next activation's perspective is installed only after the settled hold.

On a human turn, the renderer computes the acting controlled player's walkable paths and full nameable target set from the overlay state. The human can compose and revise a path, including the empty stay path, and click any visible living enemy to select its stable enemy-roster target value. Clicking the selected enemy again restores automatic targeting, represented by `target: 0`. The selection remains while the person edits the path or resets movement, and a new activation clears it. Confirm sends the selected path and target value. From the projected final tile, the informational ink thread previews the actual strike: an in-range selected target takes precedence, while an out-of-range selected target keeps its selection, changes its drawn ring from gilt to the existing ember warning color, and shows a diagonal slash at the target while the preview continues using the ruleset's automatic strike. Moving back into range restores the normal gilt marker. With automatic targeting, the preview shows a unique nearest in-range enemy, marks tied nearest candidates as uncertain, or shows nothing when no enemy is in range. The preview neither sends an action nor advances the match. The full target set remains part of the action-mask contract for agents. Spectators and replay viewers receive no action sender and cannot select targets.
