...
Each step revises the specification files it changes, in the same change set as the code. The step files name their own. `uv run python scripts/ci.py docs` only runs `mkdocs build --strict`, so it catches a broken link and not stale prose; the specification edits are a review responsibility rather than a check the pipeline can make.
...
`EnvironmentMeta` carries one tagged `layout` value with two variants: **player bounds** for the common case and **seat plans** for the rest. The tag is the declaration style, so an environment cannot supply both or neither. That is a shape the type system rules out rather than a rule a validator enforces, and it travels unchanged to the generated JSON and to the TypeScript union, so neither language needs a cross-field check for a mixed or empty declaration. What each variant carries still needs validating, and that happens at load time as described below.
...
- **Chat.** The environment may derive ordered direct-recipient choices and a default from live game state. Broadcast remains available in every messaging environment. The state carries those choices with the acting sender and a turn token. The harness, not the browser, enforces the sender, token, and current policy for human messages and applies the same recipient policy to agent output.
...
- The chat input is enabled only for the designated human player's current token. The harness rejects an inactive sender, stale token, or disallowed direct recipient, while broadcast remains available and records that acting player as its sender.
...
- Every recording header written by this stage carries the seat map, the player count, and the plan key. None of the three is optional at the read boundary, and a header missing any of them is malformed.
