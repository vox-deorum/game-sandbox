...
`frontend/src/components/admin/SeasonConfigEditor.vue` receives that count as a prop and passes it, the draft's seat compositions and game counts, and the environment down to `frontend/src/components/admin/SchedulePreview.vue`, which owns the projection. The preview recomputes whenever any of those change, which covers the environment, any layout-affecting parameter including `players` or `seat_plan`, a seat spec, and a games value. It depends on nothing else in the editor, so an invalid field in an unrelated block such as an LLM limit never blanks it. Display the exact total near the match controls, with per-match submitted-assignment and all-Naive components in supporting text. The total disappears only when no seat layout resolves, which happens while the environment metadata is still loading or a parameter is invalid. Every other failure is a typed projection error, including a stale match seat count and an invalid game count, and renders its reason. The supporting text labels eligibility as of page load and explains that triggering freezes a fresh roster. Nothing here creates a run, freezes a snapshot, or stages a submission, because the draft never leaves the browser.
