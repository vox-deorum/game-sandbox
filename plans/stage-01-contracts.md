...
- `gateway/` holds LLM gateway deployment configuration (filled in from Stage 8).

Define the per-step state object as a JSON Schema (draft 2020-12) with an explicit schema version field, covering the fields named in [interaction.md](../specs/interaction.md): tick number, per-agent display observations, actions, rewards, cumulative scores, environment-specific overlay fields, messages, and timing. Messages and overlay fields are present in the schema from day one even though messaging arrives in Stage 7, so the schema does not need a breaking revision later.
