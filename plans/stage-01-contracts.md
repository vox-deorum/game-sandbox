...
Status: complete, with GitHub Pages deployment intentionally deferred. Everything that can be built and verified locally is done: the repo skeleton and tooling, the two schema files, the `harness` package, `scripts/generate.py`, the `schema/ts` package, all three CI workflows, the docs site, and the template, `examples/hello`, compose script, and publish script. The cross-language round trip, the staleness/determinism check, the strict docs build, the composed-example tests, and the publish dry-run all pass, runnable as one command (`scripts/ci.py all`); all six workflow jobs have additionally been run green in containers with `act`. The full publish pipeline has now run for real: the `template-publish` workflow, dispatched for v0, published the placeholder template to `vox-deorum/game-agent-template` (its `main` branch and mirrored `v0` tag) and the composed `hello` example to its `examples/hello` branch, then stamped the `template-v0` tag on this monorepo. The `TEMPLATE_REPO_TOKEN` secret lives on the `template-publish` environment, which only the `publish` job declares, so the token stays gated behind environment protection rules and out of every other job. The one piece held back is GitHub Pages: the docs site is not public yet, so the `deploy` job in `docs.yml` is temporarily disabled while the strict `build` job keeps running on every push and PR; enabling Pages with the GitHub Actions source and re-enabling `deploy` is the remaining step, taken when the site goes public.
