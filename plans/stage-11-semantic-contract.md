...
Shared card code is split by responsibility. A standard-library-only `environments/src/local_play/card_utils.py` owns the integer codec, suit and rank accessors, and display names. The pure Hearts and Spades rules engines import from it without gaining NumPy or Gymnasium dependencies. `environments/src/local_play/card_spaces.py` imports that codec and declares `CARD`, `HAND`, and `TRICK`. Both modules sync into every template beside each template's own game-specific `sandbox/cards.py` helper module, which imports the shared codec.
