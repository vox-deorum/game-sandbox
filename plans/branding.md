...
The shared chrome uses one deployment brand from `GET /api/config`: the full name, compact name, and icon URL. The bundled opaque app icon centers an oversized magical flame on an uninterrupted oxblood field, with a raised inset frame for depth in the sidebar and mobile bar. A separate 32 px favicon reduces the mark to the oxblood field and flame. The approved detailed artwork remains available as `game-sandbox-icon-full.png` for future large-format use. `SITE_ICON_URL` accepts a root-relative path or an absolute HTTP(S) URL so another deployment can use its own hosted artwork without rebuilding the frontend. A custom icon also becomes that deployment's favicon.
