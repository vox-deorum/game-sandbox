...
- **Docker-free backend (Vitest, `:memory:`, FakeDriver):**
  ...
  - Admin API, gating, public season index, and WebSocket log-stream relay against a stub runner (step 3): public release filtering, the restricted public season DTO, separate submit/play targets, `open_season_exists`, `open_play_season_exists`, `empty_schedule`, the force-confirmed destructive config/deps edit, the always-editable rating-prompt route, and `is_operator`.
