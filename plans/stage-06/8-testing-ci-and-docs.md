...
- **Docker-free backend (Vitest, `:memory:`, FakeDriver):**
  - Storage (step 1): migration, renamed iteration index, the one-open-submission invariant, the one-play-open invariant, the config codec (including zero-slot and non-positive-game rejection), session iteration attribution, trigger-time roster and schedule snapshots, concrete agent refs, automated placements, ratings, both rating prompts (the operator prompt editable after a run, the author prompt surviving resubmission), protected leaderboard recordings, and the operator predicate.
  ...
  - Admin API, gating, and WebSocket log-stream relay against a stub runner (step 3): public release filtering, separate submit/play targets, `open_iteration_exists`, `open_play_iteration_exists`, `empty_schedule`, the force-confirmed destructive config/deps edit, the always-editable rating-prompt route, and `is_operator`.
