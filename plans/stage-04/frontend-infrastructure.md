...
Failures the UI must distinguish come back as typed results, not thrown strings: 403 not-allowlisted and 409 already-active. The backend's 409 body carries the active session's id, so the UI can offer "rejoin" instead of a dead end. Concretely, the error body grew a stable machine `code` (`not_allowlisted`, `already_active`) that the client branches on, plus the `active_session_id` merged into the 409 body. On the backend this is `OrchestratorError` carrying an optional `code` and a `details` object, with `replyError` spreading both into the JSON. The start route validates in order: the request first (400 for an unknown environment or an invalid mode), then the allowlist (403), then the one-per-user rule (409). A malformed start is therefore still a 400 regardless of identity.
