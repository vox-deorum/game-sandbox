...
2. **[Simulation engine and PettingZoo environment](stages/2-engine-and-environment.md).** The whole ruleset as a Python engine, movement resolved by pymunk, with the platform face on top: spaces, seat plans, presets, chat policy, compact overlay, `naive`, and `scripted_visitor`, running on a fixture village, with the shared `props.json` and `rules.json`. It registers the minimal real stub the following stages extend. Hands-on: full cast_5 and cast_10 days record to JSONL inside the size budget and replay identically.
3. **[Renderer, collision overlay, watch and replay](stages/3-renderer-and-registration.md).** The village on a shared tiled-map base (pixi-tiledmap, common to all environments) under a placeholder tileset, the permanent collision overlay above it, and the e2e group. It replaces the stage 2 stub with the real renderer and watch surface. Hands-on: watch a live match in the browser, scrub its replay to exact frames, and run a local watch-mode day.
