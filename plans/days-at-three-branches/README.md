...
Build everything the [environment specification](environment.md) calls the platform implementation: the village generator for [village.md](village.md), the simulation engine for the [ruleset](ruleset.md), the PettingZoo environment with its metadata, presets, and both builtins, the `three-branches-village` renderer, human visitor play, the template layer with the physics helpers, the canonical student guide, one worked example, and the Season 5 dialogue helper. Step 1 also carries the platform contract expansions the design assumes, because this is the platform's first simultaneous production environment and its first mask-free `Dict` action space.

Two decisions bind the whole plan:

- The visitor is `player_0` in every plan: seat_0, the cast, holds `player_1` upward, and seat_1, the visitor, holds `player_0`. Seats may group non-adjacent players, so the platform supports this today. `human_players` is the single entry `player_0`, which resolves correctly in both plans without any new mechanism. The specification's seat tables still show the visitor last and are updated at their source.
...
The completed platform stages, in particular named builtins and restricted seats (16), simultaneous stepping (17), the canonical zod contract (18), and composite action spaces (19), plus the LLM gateway's shipped proxy, credentials, and budgets for step 8. The simultaneous paths exist end to end but have never carried a production environment; steps 2 and 3 exercise them for real. Nothing here blocks on skirmish-crane, which continues in parallel.
...
8. **[Season 5 dialogue helper](stages/8-llm-dialogue-helper.md).** The background-request helper that carries LLM dialogue across ticks, with a worked pattern. Ships before Season 5 needs it; its exact timing is a course-ops choice recorded in the step file.
