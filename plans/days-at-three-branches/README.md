...
Four decisions bind the whole plan:

- The village is a grid. Characters move continuously in metres, and the static map is a grid of square cells: a cell's ground class carries its speed and its passability, so the map is one indexed lookup. Water is solid by the cell. Structural props supply walls and doorways, while interactive props and solid scenery reserve cells and carry their own box or circle collision shape. pymunk resolves movement over all of them.
...
- Shared game data lives in `rules.json` (the village frame, ground classes, emote order, character profile, phases, day length) and `catalog.json` (the building, interactive-prop, structural-prop, and scenery types). [ruleset.md](ruleset.md) is the canonical human-readable catalog. The Python engine and TypeScript renderer read both files without enumerating supported types. A data-only type works when it reuses an existing transition, placement, and art mechanism. Generator-only tuning lives in `generation.json`, read by the Python generation package.
