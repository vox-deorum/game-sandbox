...
Three decisions bind the whole plan:
...
- Player ids stay inside the environment. Observations, helpers, and student documents speak character ids alone, `npc_0` through `npc_9` and `visitor`, so a student learns one id vocabulary rather than two orderings of two.
- Shared game data lives in `props.json` (the prop catalog) and `rules.json` (emote order, ground speeds, character profile, phases, day length). The Python engine and TypeScript renderer read both files. Generator-only tuning lives in `generation.json`, read by the Python generation package.
...
2. **[Simulation engine and PettingZoo environment](stages/2-engine-and-environment.md).** The whole ruleset as a Python engine, movement resolved by pymunk, with the platform face on top: spaces, seat plans, presets, chat policy, compact overlay, `naive`, and `scripted_visitor`, running on a fixture village, with shared `props.json` and `rules.json`. It registers the minimal real stub the following stages extend. Hands-on: full cast_5 and cast_10 days record to JSONL inside the size budget and replay identically.
3. **[Renderer, collision overlay, watch and replay](stages/3-renderer-and-registration.md).** The village on a shared tiled-map base (pixi-tiledmap, common to all environments) under a placeholder tileset, the permanent collision overlay above it, the fitted, pannable, zoomable village on the shared camera, and the e2e group. It replaces the stage 2 stub with the real renderer and watch surface. Hands-on: watch a live match in the browser, scrub its replay to exact frames, and run a local watch-mode day.
4. **[Village generator](stages/4-village-generator.md).** The seeded layout under village.md's guarantees, grown in four owner-reviewed layers (terrain, sites, road network, accessories) behind the same contract, with fixture padding keeping every layer watchable in the browser. Hands-on: each layer reviewed in local watch sessions until signed off, ending with the guarantee suite green across a pinned seed batch.
5. **Realistic rendering, in two signed parts:** [5.1 art style](stages/5-1-art-style.md), the village's visual identity over the same tiled-map pipeline, prop animations, and phase lighting; [5.2 HUD and interaction design](stages/5-2-hud-interaction-and-camera.md), the information layer, the step 6 input specification, and the tuned zoom limits. Each merges on the owner's sign-off. Hands-on: the pinned fixture replays in the final style and can be explored close up.
