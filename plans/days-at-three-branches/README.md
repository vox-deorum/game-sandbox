...
5. **Realistic rendering.**
   - [5.0 atlas pipeline](stages/5-0-atlas.md): `presentation.json` declares the source art and transforms for each atlas cell, and `atlas build` compiles the committed runtime pages loaded only when a page has shipped consumers.
   ...
   - [5.3 visual refinement and fitted-view hierarchy](stages/5-3-visual-refinement.md): direct-colour asset revision, higher-resolution pine variants, a dedicated centered lantern treatment, character occlusion, HUD restyling, and the fitted-view district representation. The owner starts one unit at a time. Each unit rereads the approved conceptual art, stops after a Git-visible unstaged first comparison batch, and then has separate owner approvals for the complete asset sheet and code integration.
