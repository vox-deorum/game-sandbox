...
- `renderer/overlay.test.ts`: every malformed rejection mirrored from `test_overlay` (key sets, versions, `q` catalog and pose counts, 14-character records, coordinate and heading ranges, movement above one meter, use with movement, two holders of one prop, a state out of range, terminal off tick 1200), including a variable lantern roster and a target index above 35, plus decoder isolation from consumer mutation.
- `renderer/agreement.test.ts`: the TypeScript static decode equals the sidecar's Python decode, `geometry.ts` wall segments match the engine within codec precision, and every sidecar frame decodes equal.
