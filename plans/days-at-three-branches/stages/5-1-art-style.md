...
`renderer/assets.ts` validates the catalog and provides the runtime loader. The nine JSON atlas declarations resolve to twelve compiled pages because the character declaration has four layers. They record each compiled file, dimensions, tintability, consumer, mipmap flag, sprite-sheet frame grid, ordered cell names, source paths, and transforms. The runtime loader resolves only pages with shipped consumers: terrain, props, lantern, monuments, bell, buildings, scenery, the four character layers, and effects. Edit source art or its declared cell transform, then run `atlas build`; never hand-edit a compiled atlas page.
