...
`renderer/assets.ts` validates the catalog and provides the runtime loader. The nine JSON atlas entries record each source and compiled file, dimensions, tintability, consumer, mipmap flag, and sprite-sheet frame grid. The runtime loader resolves only pages with shipped consumers: terrain, props, lantern, monuments, bell, buildings, scenery, the four character layers, and effects. Edit art by changing loose frames and running the step 5.0 pack command, never by hand-editing an atlas page.
