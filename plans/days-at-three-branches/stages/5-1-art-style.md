...
`terrain-contours.ts`, `terrain-curves.ts`, `terrain-routes.ts`, `terrain-art.ts`, and `map-layer.ts` land the retained terrain: pattern-filled vector surfaces, seam treatments, inset routes, component decks, and upper wall bands.
