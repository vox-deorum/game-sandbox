...
Road and path share a smooth unoutlined seam. The road blends the darker timber value group over the ground surface at 0.58 opacity so it remains distinct from silt fields without becoming a solid ribbon, while the pale path remains subordinate. Water and bridge cells share one water surface, bridge entrances remain fixed to the grid, and bridge cells paint planks above the water. A curved shoreline uses a broad translucent reed wash beneath a narrower, more legible silt bank and fades out beside bridge decks. Architectural floor, doorway, and wall boundaries stay grid-aligned for the later Roofs unit.
...
- Contours: one pure half-edge graph includes the fixed map exterior, preserves every adjacency as one shared curve, and resolves ambiguous two-material saddles deterministically. It resamples the whole chain at 0.5 cell without retaining ordinary grid corners, applies two quarter-weight Chaikin passes, and adds at most 0.06 cell of deterministic nonrepeating smooth value noise. Only map edges, structures, junction tangents, and bridge entrances stay fixed. A segment that would leave the approved tube falls back locally toward the source boundary. The complete curve remains within 0.15 cell of collision truth, keeps every cell centre in its source material, and preserves at least 0.70 cell through one-cell corridors. A reed band 0.32 cell wide at 0.16 opacity sits beneath a silt band 0.16 cell wide at 0.36 opacity, with both fading beside bridges. Upper wall bands remain a second small tiled ground above characters.
