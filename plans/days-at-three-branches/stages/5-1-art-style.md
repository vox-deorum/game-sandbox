...
- Natural boundaries take the seam treatments: a darkened pooling band inside each field, reed, and water component, a broken grain-textured ink line with a faint bleed along every natural adjacency, and thin ink hatch lines offset onto the water, all cut away under roads, paths, and decks.
...
Build characters from shared north-facing grayscale-alpha masks in a conventional top-down-shooter projection. The camera looks straight down onto the head, shoulders, torso, arms, and partly occluded lower body. A peaceful forward-arm pose makes north readable without a weapon. Rotate the complete assembled sprite around its centre to the exact recorded heading. A rest frame and short walk cycle advance from the on-screen distance actually walked, so the cycle slows with slower movement and rests the instant movement stops, without changing that projection. Render a readable fitted-view shadow and direction mark. Select tint combinations and optional shared clothing details with a stable player-id hash. Give `player_0` a small cinnabar hood tie and retain the villagers' warm materials. The owner approved [the top-down shooter direction](../art/top-down-shooter-direction.png).

![Approved top-down shooter direction](../art/top-down-shooter-direction.png)

Every ordinary catalog state has one distinct complete north-facing still across its catalog footprint, turned to its facing. The lantern, roadside shrine, and notice board always draw fixed north, ignoring their recorded facing. Ordinary runtime prop cells use a 384 by 256 canvas that centers their footprint with at least two transparent pixels at its edge. The board keeps a 512 by 512 canonical source and is fitted proportionally into its shared runtime cell. The lantern uses complete 384 by 512 state frames on its dedicated page, with a centered 160 px silhouette scaled by `0.10` to its one-cell collision footprint.

The shared canvas usable area is 380 by 252 px, which at the 0.14 default scale carries at most 3.36 by 2.24 cells. The board's 512 by 512 canonical source carries a centered silhouette fitted within a 448 by 448 normalization target. Its configured visible-bounds fit uses the full safe cell height and produces a centered 251 by 252 runtime silhouette at scale `0.12444444444444444`, without aspect distortion or a separate atlas. This preserves the prior 31.36-unit vertical extent and gives a 31.24-unit horizontal extent, a 0.4 percent reduction caused by the source's one-pixel aspect difference. The shrine and the plot do not fit at 0.14, so their overrides are permanent until an atlas re-cut: re-author targets at their new scales, shrine 240 by 240 px at 0.20 (80 px per cell) and plot 376 by 188 px at 0.17 (94 px per cell). A 512 by 512 shared props cell would hold a 3 by 3 shrine and a 4 by 2 plot at 114 pixels per cell with no overrides; that atlas re-cut is deliberately not part of this change. The shrine's incense effect does not scale with the prop, so the smoke reads slightly smaller over the bigger shrine; that is a re-author concern, not a renderer one.

The dedicated monument atlas is the sole authority for the fixed-north pump. Its 768 by 512 masters are tightly authored around their collision registration, never by enlarging legacy transparent bounds. The bell has its own double-density 1536 by 1024 page with foundation, fixed gantry, and moving circular-bell frames. `presentation.props.registeredPropByType` stores each type's texture-density divisor and the absolute source-pixel anchor for each role. The renderer divides the configured pump scale by 4 and the bell scale by 16, then places that source anchor at the collision centre. The pump still anchor is `(344, 384)`. The bell foundation, gantry, and moving-bell anchors are `(768, 512)`, `(768, 960)`, and `(768, 960)`.

The pump is a fixed north-facing monument: its circular footing is mechanically registered to the centered 1.0-cell collision circle while the mechanism may extend north beyond it. The bell has a state-independent circular stone foundation below characters and two fixed-north roles above them: a rectangular pair of parallel posts joined by a crossbeam, plus a separate circular bell in true transparent open space. The gantry stays fixed. Only the circular bell moves while ringing.

The configured prop scale defaults to 0.14, overrides bell to 0.36 for both art parts, overrides pump to 0.33, shrine to 0.20, board to `0.12444444444444444`, and plot to 0.17, while bell collision remains a 1.0-cell circle. Drive the result only from prop id, type, state, facing, and tick.
...
| Lantern | `lit` has a gilt core and small post-grade pool; `unlit` has a dark empty lantern. Its complete lower-layer state frame, flicker, glow, contact shadow, and selection share the one-cell collision centre at `(0, 0)`. Draws fixed north, ignoring facing. |
...
| Garden plot | Both states use one flat ground-inlaid 2 to 1 boundary in a 100 percent top-down orthographic view. `tended` has ordered low green rows with visible dark soil; `overgrown` has irregular pine-green growth with restrained violet and ochre flowers. The medium-dark cedar frame follows the selected bench and notice board family and exposes top faces only, so every quarter-turn facing remains credible. |
...
| Beacon bell | `ringing` moves only the separate circular bell layer in the overhead plane; `silent` keeps it centered. The rectangular gantry and circular foundation remain fixed, with true alpha between all three pieces. Registry: 1536 by 1024 frames; foundation anchor `(768, 512)`; gantry and moving-bell anchors `(768, 960)`; scale divisor 16. [Step 5.3 unit 6](5-3-visual-refinement.md#6-monument-and-effect-completion) owns both remaining approval gates. |
...
The local manifest is the only asset catalog. Keep its referenced canonical source art in `environments/three_branches/renderer/assets/source-art/` and remove unreferenced intermediate sheets or superseded masters. Keep optimised compiled atlas files in `environments/three_branches/renderer/assets/`. The ordered cells in `presentation.json` name each source and its optional crop, resize, alpha normalization, fit, and cleanup rules; [step 5.0](5-0-atlas.md) compiles those cells into runtime atlas pages. Source art and compiled pages are committed together. Use grayscale-alpha masks for tintable textures and full-colour raster art only where tinting cannot express the treatment.

`environments/three_branches/renderer/assets/presentation.json`, validated by `presentation.ts`, owns the atlas catalog, palette, ground variants, land, water, road, and path curve profiles, seam treatments (pooling, ink, water hatching), reed marks, inset route and deck geometry, roof fade, post effects (the authored grade, the night grade, and the prop contact shadow), character treatments, prop and scenery scales, monument texture-density divisors and source-pixel anchors, prop effects, and crane dressing. Each atlas group carries its mipmap flag in this catalog. `generation.json` remains generation-only, so visual calibration cannot alter seeded layouts.

`renderer/assets.ts` validates the catalog and provides the runtime loader. The nine JSON atlas declarations resolve to fourteen compiled pages because the buildings declaration has three layers and the character declaration has four layers. They record each compiled file, dimensions, tintability, consumer, mipmap flag, sprite-sheet frame grid, ordered cell names, source paths, and transforms. The runtime loader resolves only pages with shipped consumers: terrain, props, lantern, monuments, bell, the three buildings pages, scenery, the four character layers, and effects. Edit source art or its declared cell transform, then run `atlas build`; never hand-edit a compiled atlas page.
...
| Buildings | Three mipmapped full-color one-frame pages: home 1024 by 896, inn 1536 by 1280, shed 1024 by 1024 | One centered full-roof frame per semantic building, authored at 128 px per cell for home 8 by 7, inn 12 by 10, and shed 8 by 8 |
...
| Monuments | 768 by 512 cells on one 2304 by 1024 atlas page | The fixed-north pump stills, tightly authored around their configured source-pixel anchor and scale divisor |
| Bell | 1536 by 1024 cells on one 4608 by 1024 atlas page | The double-density registered foundation, fixed gantry, and separately animated circular bell |
...
- `presentation.json` holds `atlases` (source and runtime paths, dimensions, frame grids, formats, consumers, and mipmap flags), `palette` (the 13 keys), `transition` (natural and settle-grace durations), `terrain` (fills, contour calibration, seam treatments, reed marks, planks, upper wall), `roofs` (clear alpha, fade duration, and one frame name per building layer), `postEffects` (`authoredGrade`, `nightGrade`, and `propContactShadow`), `characters` (clothing tints, details, walk, visitor), `props` (0.14 default still scale, lantern, bell, pump, shrine, board, and plot scale overrides, ordinary effect offsets, and pump and bell density divisors with source-pixel anchors), `scenery` (0.25 default sprite scale, multiplied by a box's catalog footprint or a pine's recorded size), `propEffects` (lantern, hearth, shrine, pump, bell), `emissives`, and `cranes`.
...
- Map edges, structures, and 0.15 cell of tangent on each side of them stay locked. That tangent holds three chains leaving one junction node in consistent directions and freezes the first corner past the node, so it is kept only as long as needed.
...
- All three seam treatments cut away under road, path, and deck coverage through one inverse mask.
...
`characters-art.ts` selects player-id-hashed tints and details from the allowed pool, advances the walk cycle (leftForward, pass, rightForward, pass) by the on-screen distance actually walked, and fixes rotation at 90 degrees minus heading, in radians. `characters.walk.frameRatio` gives each pose's walked distance as a positive fraction of one moved unit, so the cycle slows with slower movement and rests the instant movement stops. Sub-dead-zone displacement, below `characters.walk.deadZone`, rests the feet. A stop that lands while a movement is still gliding settles the remaining stretch with resting feet instead of popping, and a reconnection that re-delivers the current tick leaves the glide and the camera untouched. A replay seek re-anchors the walk phase to the seeked frame, a deterministic snap like the expression chips. `characters.ts` assembles each character as a shadow plus a rotor of body, clothing, arms, and detail masks with a direction mark. `player_0` wears `visitorTie` in cinnabar. Below the far-view readability threshold shared with [step 5.2](5-2-hud-interaction-and-camera.md)'s nameplates, a character draws as a Hearthside-styled overhead mark, a tinted circle with a direction tick, in place of the unreadable sprite. Step 5.2 owns the ungraded recorded-expression treatment, and [step 5.3 unit 5](5-3-visual-refinement.md#5-four-layered-cast-sets) owns the final layered cast revision.
...
`props-art.ts` resolves every catalog type and state to its north-facing art roles. Ordinary props use 384 by 256 runtime cells; the board cell is fitted from its 512 by 512 canonical source. The pump uses dedicated 768 by 512 monument frames. The bell uses 1536 by 1024 foundation, gantry, and moving-bell frames on its dedicated page. `props-layer.ts` divides the pump scale by 4 and the bell scale by 16, applying each configured source-pixel role anchor so tight masters preserve collision-centered world bounds. `SHIPPED_PROP_TYPES` enables all ten complete prop treatments. The pump ignores placement facing, keeps its centered shadow below characters, and draws its mechanically registered complete still and water effect in the upper character layer. Its water anchor remains in the presentation configuration's original 384 by 256 coordinate system and is multiplied only by the configured pump scale. The bell adds its state-independent foundation to the prop layer below characters, while its fixed gantry, moving circular bell, and line effect stay in the upper character layer. `effects.ts` holds the five sustained accent animations, the bell swing, and emissive specs as pure functions of fractional tick, prop id, state, and stable hash phase. Effect configuration objects own their frames and `frameRate`. `props-layer.ts` applies the configured 0.25 scenery baseline and the catalog footprint to the 2 by 2 market crate, validates the shipped still and needed accent frames before installation, and installs art atomically.
...
Deferred to [step 5.3 unit 6](5-3-visual-refinement.md#6-monument-and-effect-completion). Retain this acceptance rubric: inspect the centered 1.0-cell collision circle at fitted, middle, and close zoom, then ring the bell through the normal Use action. Accept only a solid civic plinth below characters, a fixed-north rectangular two-post gantry above them, clear alpha apertures around the separately layered circular bell, a stationary silent state, a restrained seek-safe ringing swing, and a restrained six-frame gilt bell-line cadence. The foundation must not read as a well, the gantry must not form a triangle, and no white backdrop may appear through the frame.

## Roofs

`buildings.ts` keeps the outline rectangles as the pre-art fallback and, once the buildings atlas installs, replaces them with one centered full-roof sprite per semantic building, built once into retained roof containers inside the authored composite above characters. The single layered `buildings` declaration provides one frame name per page layer: `home`, `inn`, and `shed`. Those full-color pages are mipmapped and authored at 128 px per semantic cell: home is 1024 by 896 for its 8 by 7 rect, inn is 1536 by 1280 for its 12 by 10 rect, and shed is 1024 by 1024 for its 8 by 8 rect. Runtime scales each sprite by `cellSize / 128` and rotates it by the building's facing around the semantic rect center. The roofs container is retained across the art-load layer swaps, which replace only the character and upper-wall layers in place. Occupancy of the semantic rect at the recorded target tick sets target alpha to `0.18` when occupied and `1` when empty. The between-tick clock eases toward the target over `220` ms, including a reversal from the current alpha. Seek, mount, frame repeat, resize, and re-installation snap. Roofs land after characters so the fade is reviewed over visible occupants. Roof occupancy reads recorded positions, so a character interpolating across a rect boundary can briefly clip an opaque roof; that is a known limit of recorded occupancy.

Tests cover the one-frame page names and dimensions, centered full-roof sprite placement, `cellSize / 128` scaling, facing rotation around each semantic rect center, occupancy targets, `0.18` and `1` alpha endpoints, `220` ms easing and reversal, build-once install and pre-install no-ops, snap semantics, and preflight failure leaving the fallback intact. The owner reviews the home, inn, and shed roofs fading on entry.
...
- Cadence: a paced host keeps the natural duration times its `transitionScale`. With no scale, the renderer measures the wall-clock gap between deliveries and animates over an EMA-smoothed estimate of the gap with a small headroom capped at the natural duration. The frame loop stays alive for a short grace after settling.
...
- Configuration: `presentation.json` validation for contour, seam, route, deck, one-frame roof names and dimensions, and post-effect calibration, the 13 fixed hexes, and the paced and unpaced duration rules.
