...
| Well pump | Its round well curb is mechanically registered to the centered 0.6-cell collision circle and its fixed-north mechanical assembly extends north beyond it. `flowing` has a visible pale water stream and wet basin mark. Its configured `(31, -61)` source-pixel effect anchor is multiplied by the configured pump scale, and the current stills carry a 4-pixel downward registration adjustment; `idle` has a dry basin and upright handle. |
| Beacon bell | Its state-independent filled circular stone foundation is centered on the 0.6-cell collision circle below characters. Its fixed-north timber bell upper assembly extends beyond it above characters. The configured 0.14 default overrides both parts to 0.36. The registered `ringing` upper has a tilted bell and exposed clapper, while `silent` hangs plumb. Manual owner review remains pending. |
