...
- Contours: one pure half-edge graph includes the fixed map exterior and preserves every adjacency as one shared curve. Components retain their outer ring and directly owned holes; islands remain separate components without containment metadata. Before shaping, each chain derives a smoothed reference polyline. The raw boundary traces cell edges, so a boundary that really runs at a shallow angle arrives quantized into stair runs of whatever length that angle implies, and the reference recovers the line those runs approximate at every run length. It samples the raw boundary every cell and fits a quadratic through a window of five samples either side, four times over: a quadratic reproduces a bend of that shape exactly and removes only the quantization rhythm it cannot follow, and repeating a modest window reaches past a long stair run without the overshoot a wide window develops on a winding bank. Each free sample then stays within 0.55 cell of the raw position it was sampled from, half a step further than a quantized straight run ever needs, so the bound only ever costs a genuinely sharp feature such as a one-cell inlet. Every reference sheds the same staircase, so the two banks of a thin band travel together and keep the width between them; three passes measure the distance that actually remains and pull back only the points that fell inside the 0.70-cell corridor, by half the shortfall each, since the competitor yields the other half on its own turn. Locked geometry keeps its exact raw shape, and a closed reference keeps a vertex at raw offset zero so downstream seam interpolation stays valid. Shaping runs per chain on the reference: a three-point corner kernel smooths the resampled polyline over the configured passes, then summed noise octaves displace free points along the local normal. Land uses 0.25-cell samples, eight passes, and octaves of 0.28 cell at eight cells, 0.12 at three, and 0.05 at 1.2. Water uses 0.20-cell samples, ten passes, and octaves of 0.34 at eleven, 0.14 at four, and 0.06 at 1.5. Closed chains cap their effective passes by sample count so small ponds keep their shape, and a profile configured past twelve passes blends back to its twelve-pass form near locks so junction approaches stay natural. Map edges, structures, junction tangents, and bridge entrances stay locked. Displacement is measured perpendicular to the chain's own reference within a 2.5-cell arc window, so a curve may slide along its boundary freely while a clearance envelope caps how far it leaves it: at most the 0.6-cell deviation cap, at most half the slack beyond the 0.70-cell corridor to competing reference boundaries, and at most the arc distance to the nearest lock, with the envelope evaluated at both the sample and its projection. Same-chain segments compete only when their arc distance is clearly larger than their straight-line distance, so a chain smooths freely along itself while inlets and hairpins keep their corridor. Rare tip geometry that still smooths across itself is repaired directly: crossing pieces pull halfway toward the reference until the curves are planar. A final validator proves the reference tube, with an interpolation sag allowance, and the intersection-free topology.
