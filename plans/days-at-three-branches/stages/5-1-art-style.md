...
Paint the engine-authored `overlay_static` grid as continuous material surfaces. The gameplay and collision grid stays authoritative. Natural visual boundaries may move by at most 0.35 cell from their source edge, stay at least 0.15 cell from every source-cell centre, and retain at least 0.70 cell between opposing boundaries. This lets open regions relax further than narrow corridors without changing collision or recorded ground.

Each material keeps several deterministic per-cell texture variants. Before contouring, a renderer-only substrate pass propagates the nearest natural ground, field, or reed material through road and path cells, with stable ties. A pure shared-contour pass traces every resulting material adjacency once and gives both neighboring materials the same curve in reverse. Land, water, road, and path use one curve-profile vocabulary: sample spacing, macro window, fairing iterations, fairing radius, fairing strength, noise amplitude, and noise wavelength. Sparse tiled layers extend one cell beyond their semantic regions and clip through retained vector masks, preserving native 128 px detail without a repeating world texture. Ground covers the complete map beneath them.

Road and path become inset textured routes above that substrate. The guaranteed west-to-east road follows deterministic column-run medians at 2.10 cells wide, may narrow no further than 1.60 cells, and blends the timber treatment at 0.82 opacity. Each pale path is a 0.70-cell stroke at full opacity. Water and bridge cells share one water surface. Bridge components retain exact portals, classify as road or path with road winning a mixed contact, and clip their plank texture through component-wide deck masks at the matching route width. Shoreline marks are broken reed and silt strokes clipped to the non-water side and tapered beside bridge decks. Architectural floor, doorway, and wall boundaries stay grid-aligned for the later Roofs unit.
...
`environments/three_branches/renderer/presentation.json`, validated by `presentation.ts`, owns the palette, ground variants, land, water, road, and path curve profiles, shoreline bands, inset route and deck geometry, roof fade, phase grades, prop effects, and crane dressing. `generation.json` remains generation-only, so visual calibration cannot alter seeded layouts.
...
`terrain-contours.ts`, `terrain-routes.ts`, `terrain-art.ts`, and `map-layer.ts` land the retained terrain: sparse textured surfaces, shared vector masks, inset routes, broken shoreline, component decks, and upper wall bands.

- Surfaces: the full ground base covers every cell. A nearest-natural propagation fills road and path cells with renderer-only ground, field, or reed substrate before natural contours are built. Stable per-cell texture variants and one-cell halos preserve native detail without creating a repeating macro texture. Water and bridge share one visual surface; exact structure tiles sit above the natural layers.
- Contours: one pure half-edge graph includes the fixed map exterior, preserves every adjacency as one shared curve, and resolves ambiguous two-material saddles deterministically. Land uses 0.25-cell samples, no macro window, four radius-1.5 fairing iterations at strength 0.35, and 0.04-cell noise with a five-to-nine-cell wavelength. Water uses 0.20-cell samples, a four-cell macro window, six radius-2.5 fairing iterations at strength 0.45, and 0.06-cell noise with a seven-to-twelve-cell wavelength. Map edges, structures, junction tangents, and bridge entrances stay fixed. Each free interval tries deterministic blend levels from full treatment through raw geometry. Local violations lower that interval; pair and global violations lower both incident intervals in sorted id order. The final curves stay inside the 0.35-cell cap, preserve 0.15-cell centre clearance and 0.70-cell corridors, and retain the shared topology.
- Routes: the road uses deterministic column-run medians, including road-owned bridge components. Its curve profile uses 0.25-cell samples, a 1.5-cell macro window, four radius-one fairing iterations at strength 0.35, and 0.05-cell noise with a four-to-eight-cell wavelength. It is 2.10 cells wide, never narrows below 1.60, and renders at 0.82 opacity. The path profile uses 0.20-cell samples, a three-cell macro window, six radius-two fairing iterations at strength 0.45, and 0.04-cell noise with a five-to-ten-cell wavelength. Paths render as 0.70-cell strokes at full opacity. Repeated micrograin is clipped through each stroked vector mask.
- Shoreline: centered strokes are clipped through the non-water mask. The reed wash is 0.60 cell wide at 0.14 opacity, 0.65 density, and 1.5-to-four-cell runs. Silt marks are 0.24 cell wide at 0.24 opacity, 0.35 density, and 0.75-to-2.25-cell runs. Stable arc-length noise breaks both bands, and both taper over 0.35 cell beside bridge portals.
...
- Keep the existing Terrain atlas dimensions, frame names, and compatibility frames. Repaint the wash, road, furrow, reed, ripple, and bridge families from the approved material study. Tiled fill frames remain opaque with matching borders, and their same-hue value detail stays within 7 percent of the configured tint. Bridge masks keep their transparent water gaps. Static substrate, contours, routes, shoreline, and decks build once during art installation and do no tick-time work.

Tests cover strict curve-profile configuration, contour topology and determinism, full partition ownership, saddle and junction routing, islands and holes, adaptive deviation, centre clearance, corridor width, substrate propagation, route ownership and widths, shoreline clipping and gaps, component deck masks, and the sparse-layer lifecycle. Run the Three Branches e2e group here, since the scene-graph reshuffle and the first textured layer land together.
...
- Configuration: `presentation.json` validation for contour, shoreline, route, and deck calibration, the 13 fixed hexes, day-grade neutrality, and the paced and unpaced duration rules.
