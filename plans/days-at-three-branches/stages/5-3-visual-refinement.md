...
The bridge pass keeps the terrain and dedicated bridge source atlases unmipmapped. Runtime rendering builds one mipmapped canvas for each connected bridge component from the shared visual deck geometry and existing component mask. It owns the board layout, direct colour composition, deterministic source phasing and end reversal, internal seam underlays, visible portal and side overlap, and centered visual weight, while the extracted full-colour sources provide material construction and local marks. Axis decks visibly extend 0.50 cells into each route landing without changing gameplay geometry. Their source canvases include another 0.20 cell of hidden run-axis padding beyond the shared mask, while the route cutout ends 0.20 cell inside the visible deck edge. The road therefore continues beneath the outer landing instead of shrinking away from it. A restrained alpha-contained shadow softens both cross-axis edges without adding a shadow to the route contacts. The dedicated `boards` frame contains exactly three vertical plank samples extracted by scanning columns. Runtime removes only their empty end margins before applying the configured phase and overscan. A horizontal deck run draws those vertical samples without rotation. A vertical deck run rotates them 90 degrees. Compact components use the same samples and rotate them according to their deterministically selected run axis. This contract is visual orientation only and does not change gameplay geometry, route ownership, water semantics, collision, or layer order.
