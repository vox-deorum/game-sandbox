...
- Water collision chains: each channel's two bank lines as static segments, with a gap where a bridge deck crosses and short rails along the deck sides. Water needs no special rule in the solver; banks are ordinary static geometry, and decks are the authored gaps in it.
...
- Static solids: wall segments, the four boundary walls, scenery circles, prop footprint boxes, and the water bank chains. A body may overhang the water line by up to its radius, which is intended: it is what lets two characters pass on a 2 m deck.
...
1. Degrade commanded values per the ruleset: a heading of 360 wraps to 0, and the default order is speed 0, heading unchanged, expression none.
...
- `decode_overlay` is the strict authority for replay consumers: exact key sets, record lengths, the fixed counts, coordinate and angle ranges, grid rows summing to 100, expression and target consistency (a use implies moved 0, no two characters hold one prop), and terminal exactly on tick 1200, decoding to friendly JSON in meters and words.

The estimate is 5.5 to 6 KB of overlay per frame and roughly 9 MiB per recorded cast_10 day against the 10 MiB budget. The budget test measures the real number here and step 4 re-measures on the generated village; if it breaches, the named mitigations are 2 m grid cells and tighter scenery records.
...
### Registration and the stage stubs

Discovery is all-or-nothing: a package under `environments/` is either listed in `.envignore` or subject to the full conformance suite, whose authoring-shape test requires `environment.md`, a renderer with exactly one thumbnail, a template, and at least one example, and whose renderer-ownership test forbids a `renderer/` directory on an ignored package. So the package sits in `.envignore` with no renderer directory while the engine milestones land, and the final milestone registers it whole:

- Remove the `.envignore` line and ship the minimal real stubs later stages replace behind the same contracts: a trivial `renderer/index.ts` with the key `three-branches-village` and a thumbnail (step 3 replaces the body), a raw-observation stand-still `template/agent.py` with its README (step 7 replaces both), an embryonic `examples/sweeper/` smoke example under the name step 7 grows into the real worked example, and an `environment.md` factual stub (step 7 grows it into the guide).
...
7. Registration and conformance: the `.envignore` removal, the stubs, `sync:envs`, the Dockerfile lines, the compose pin, the full conformance suite green for three_branches defaults, and the cast_10 seconds-per-tick measurement recorded.
...
- `test_overlay`: encode and decode round trips, each malformed field rejected, the static section byte-identical across frames, the recording size budget on a full cast_10 day, and replay determinism through the harness (two identical seeded rollouts compared without timing fields).
...
- `test_budget`: a full day fits the per-decision and per-game compute budgets, and a measurement of real seconds per tick for a cast_10 day is recorded against the 250 millisecond cadence, with engine transition time measured separately from agent decision time, since the engine's cost is uncharged platform time that still consumes the cadence.
...
Full cast_5 and cast_10 days run through the harness on the fixture village, record inside budget, and replay identically, the conformance suite is green for three_branches defaults, and the registration milestone has landed whole: catalog entry and stubs, Docker smoke lines, and staged builtins.
