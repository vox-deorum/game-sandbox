...
| `generation.json` + generation loader | Immutable generator tuning and its import-time validating loader |
| `geometry.py` | Closed-form primitives for the non-physics rules: heading wrap and vectors, distance, nearest point on a rotated rectangle, cone test, segment intersection, point in polygon and rectangle |
...
| `generation/` | `build_village(seed) -> Layout` and the terrain, sites, network, accessory, and validation stages that fill the seam |
...
`environments/three_branches/props.json` holds the prop catalog as a `props` array in the ruleset's table order. Each record carries `token`, `title`, `activity`, `states`, `start`, `transition` (`{"kind": "toggle" | "occupancy" | "timed" | "none"}` with `ticks` present exactly for timed), `footprint` (`{"width", "depth"}` in meters), `count`, and `district` (a placement token for step 4's generator). Fixed types have positive counts. Only lantern uses `count: null`, allowing its road-derived count to vary. The loader validates snake_case unique tokens, states within the observation charset with `start` among them, the transition shapes with exactly two states for every stateful kind and a resting second-state start for occupancy and timed props, and positive footprints and fixed counts. It exposes `PROP_TYPES` in canonical order.
...
The rules and prop loaders run at import, the way Skirmish at Crane Reach loads `tile_types.json`, and step 3's renderer imports the same two JSON files by relative path. The generation loader also runs at import, but `generation.json` is Python-only maintainer tuning. It strictly validates the pipeline, walker, terrain, sites, network, and nested accessory groups before any seeded layout is built. Accessory values live with their object, such as `accessories.pine.radius` and `accessories.lantern.spacing`.

### Layout types and the fixture village

`layout.py` defines the static layout in the shape of the observation's `village` Dict: channels (trunk first), road, and footpaths as centerline-and-width polylines, bridges as center, heading, width, and span, buildings as rectangles with a doorway, fields and reed banks as polygons, a variable prop sequence with rotations, scenery circles, and the spawn point. From these it derives, once per layout:
...
The version 1 overlay uses the recording contract's static and dynamic split: `overlay_static` is captured once after reset in the header, and each step carries only its dynamic data. Both use single-character keys, base36 fields, positions in integer centimeters (three characters per coordinate), angles in tenths of degrees (three characters), and small lengths in centimeters (two characters).

- Header `overlay_static` holds the static layout: channels, road, and footpaths as width plus packed points, bridges, the seven buildings in canonical order with ids implicit, `q` as two base36 count digits per catalog type, props as position and rotation with footprints implicit through `props.json`, scenery, the spawn, and 100 run-length-encoded ground grid rows (1 m cells sampled at cell centers through the engine's `ground_at`, coded per `rules.json`), which step 3's tile renderer consumes directly. Counts derive ids in catalog order, require every fixed catalog count, allow the lantern count, and cap the roster at 1,295 props.
- The dynamic section: the tick, one 14-character record per character in roster order (x, y, heading, moved, expression id, and a two-character use-target prop index, with `zz` for none), a prop-state string matching the static roster length, and the terminal flag. Bell and phase are derived on decode, from the bell prop's state and from the tick plus the daynight flag.
- `decode_overlay` is the strict authority for replay consumers and takes both dynamic and header-static data: exact key sets, record lengths, catalog and pose counts, coordinate and angle ranges, grid rows summing to 100, and expression and target consistency (a use implies moved 0, no two characters hold one prop), decoding to friendly JSON in meters and words. A terminal frame must name tick 1200. The nonterminal state that offers the final action also names tick 1200, so both consecutive states are valid.

The current cast_10 fixture recording is 1,978,650 bytes across 1,201 lines. Its header is 5,850 bytes, with a 4,729-byte canonical static payload. Tests cap the recording below 2 MiB and the full header below 16 KiB. The full-day timing case keeps each engine transition plus serial charged agent work below the 250 millisecond cadence, each builtin call below its step budget, and reset plus all calls below each agent's episode budget.
...
1. Data and geometry: the package skeleton behind `.envignore` with pymunk pinned, the shared JSON files and loaders, generator-only `generation.json` and its loader, `geometry.py` with exhaustive unit tests.
2. Layout and fixture: the layout types, derived walls and bank chains, the ground classifier, the fixture village, and the `generation` seam.
...
- `test_rules_data`: the rules, prop, and generation loaders reject malformed documents, and shipped values are pinned against the ruleset and generation configuration. The fixture may retain its 31-prop roster.
...
- `test_overlay`: encode and decode round trips, each malformed field rejected, version 1 static data separated from every dynamic frame, and decoder isolation from consumer mutations.
- `test_builtins`: naive performs the same random walk for the same seed, changes heading, recovers from stalls, and stays finite through the real engine; staged copies are byte-equal for both builtins; neither imports the engine or any third-party package; and the scripted visitor is deterministic per seed, wanders, approaches, and speaks its canned lines.
- `test_budget`: full cast_5 and cast_10 days each produce 1,201 recording frames and replay identically from the same seed. The cast_10 recording stays below 2 MiB, and its header stays below 16 KiB. Each real builtin's combined `act` and optional `chat` fit the per-step budget, while `reset`, `act`, and `chat` together fit the per-game budget. Each tick's serial charged agent work and engine transition must stay below the 250 millisecond cadence; reset is not part of a gameplay tick.
