...
- `layout`: the static village, `ground_at`, `walkable`, `can_step`, `line_of_sight`, `buildings`, `building`, `doorway`, `spawn`, and `SPEED_LIMITS`. Movement and perception stay apart, because the ruleset keeps them apart: `walkable` and `can_step` test a body against everything solid, water, the boundary, wall segments, solid scenery, and prop footprints, while `line_of_sight` is the perception rule, where only walls cut a line.
...
- `layout.ground_at`, `layout.walkable`, `layout.can_step`, and `layout.line_of_sight` pinned against the engine's own ground classifier, movement collision, and sight rules across a pinned seed batch, so the helpers can never drift from the physics. `can_step` agrees with where `env.step` actually stops a body, including against water, solid scenery, and prop footprints, and `line_of_sight` agrees with what the observation actually shows.
