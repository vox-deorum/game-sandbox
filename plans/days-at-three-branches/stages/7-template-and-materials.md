...
- `layout`: the static village, `ground_at`, `walkable`, `can_step`, `line_of_sight`, `buildings`, `building`, `doorway`, `spawn`, and `SPEED_LIMITS`. Movement and perception stay apart, because the ruleset keeps them apart: `walkable` and `can_step` test a body against the static solids, the water banks, the boundary, wall segments, solid scenery, and prop footprints, ignoring characters, while `line_of_sight` is the perception rule, where only walls cut a line.
...
- `layout.ground_at`, `layout.walkable`, `layout.can_step`, and `layout.line_of_sight` pinned against the engine's own ground classifier, static collision geometry, and sight rules across a pinned seed batch, so the helpers can never drift from the physics. `can_step` is the static clearance test: a step it approves crosses no water bank, wall, solid scenery, or prop footprint, pinned against the engine's collision shapes, and `line_of_sight` agrees with what the observation actually shows.
