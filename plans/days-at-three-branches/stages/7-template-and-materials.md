...
- `props`: `all(observation)`, every interactive prop's id, type, cell, facing, and owner where applicable as standing knowledge; `seen(observation)`, the ones whose state is currently perceived; `in_reach(observation)`; `usable(observation)`, the prop a use would take on this tick; and `TYPES`, the interactive catalog from `catalog.json`.
- `layout`: the static village, `frame`, `cell_at`, `ground_at`, `walkable`, `can_step`, `line_of_sight`, `buildings`, `building`, `doorway`, `spawn`, and `SPEED_LIMITS`. Static records omit shape and footprint dimensions. The helpers derive catalog shapes from type tokens, including structural walls and doorways. Movement and perception stay apart, because the ruleset keeps them apart: `walkable` and `can_step` test a body against impassable ground and catalog-derived structural, interactive-prop, and scenery shapes, plus the boundary, ignoring characters. `line_of_sight` uses catalog opacity.
...
- `layout.ground_at`, `layout.walkable`, `layout.can_step`, and `layout.line_of_sight` pinned against the engine's own grid, static shapes, and sight rules across a pinned seed batch. The helpers derive the same catalog shapes and opacity the engine uses, so agreement is exact. `can_step` is the static clearance test: a step it approves crosses no impassable ground or catalog-derived object shape, and `line_of_sight` agrees with what the observation actually shows.
