...
- `props`: `all(observation)`, every interactive prop's id, type, cell, and facing as standing knowledge; `seen(observation)`, the ones whose state is currently perceived; `in_reach(observation)`; `usable(observation)`, the prop a use would take on this tick; and `TYPES`, the interactive catalog from `catalog.json`.
- `layout`: the static village, `frame`, `cell_at`, `ground_at`, `walkable`, `can_step`, `line_of_sight`, `buildings`, `building`, `doorway`, `spawn`, and `SPEED_LIMITS`. A building record is purely semantic, an id, a type, and an origin cell, and `doorway` reads the doorway cells off the building's own perimeter in the ground grid. Movement and perception stay apart, because the ruleset keeps them apart: `walkable` and `can_step` test a body against impassable ground (water and wall cells), catalog prop and scenery shapes, and the boundary, ignoring characters. `line_of_sight` walks the ground grid and is blocked only by wall cells; props and scenery never block a line.
...
Every function takes the observation first and is either a stateless reader or a pure builder. There is no map object to construct, reset, or carry across episodes, and no layout in module-global state: any caching is keyed on the layout content and invisible from the outside. Nothing here decides anything, and deliberately no pathfinder, the rule `sandbox.crane` already states. The map is not withheld with it: `observation["village"]` carries the whole layout as standing knowledge, and `walkable`, `can_step`, and `ground_at` are the node test, the edge test, and the edge cost a route planner needs. Step 1's setup observation gives that planner somewhere to be built: `reset(seed, observation)` hands over the layout before the first tick, and the cost lands on the episode budget rather than on one decision. Only the search is student work, and step 8 ships one worked answer.
...
- `layout.ground_at`, `layout.walkable`, `layout.can_step`, and `layout.line_of_sight` pinned against the engine's own grid, catalog shapes, and sight rules across a pinned seed batch. The helpers read the same ground grid and catalog shapes the engine collides with, so agreement is exact. `can_step` is the static clearance test: a step it approves crosses no impassable ground (water or a wall cell) or catalog prop or scenery shape, and `line_of_sight` agrees with what the observation actually shows, blocked only by wall cells.
- A route graph over the whole village, built in `reset` through `layout.walkable` and `layout.can_step` the way step 8 builds one, leaves the episode budget with ample room for a full day of decisions. Reset cost and per-tick cost are measured separately, which is what proves the package's internal caching.
- `props.usable` equals the prop `env.step` actually selects, across the reach boundary, the tie rule, the stillness rule, and a blocked line. `props.TYPES` pinned against `catalog.json` and the `geometry` constants against `rules.json`.
- Isolation: two Agent instances and two episodes receive independent plain observation mappings. Mutating one mapping cannot change another mapping or the environment's internal village snapshot, and a `reset` changes nothing a student has to remember.
