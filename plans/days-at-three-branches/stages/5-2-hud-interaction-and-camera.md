...
- A permanent virtual joystick sits in the bottom-left of the content area for pointer and touch. A primary press inside its ring engages the fixed pad. Dragging sets heading from the drag angle and relative speed from the drag distance, with a 15 percent positional dead zone and full speed at the pad ring. A drag whose relative speed falls below a small threshold (0.05, matching `characters.walk.deadZone`, the engine's normalized per-tick moved) reads as no motion, so a nearly-released thumbstick cannot feed an imperceptible crawl to the engine. Release stops the visitor and returns the knob to the center. Presses outside the pad and expression palette keep the camera gestures, including the double-click or double-tap that resumes visitor follow.
