...
4. The entry-point module file the manifest names **exists** in the tree, else `entry_point_missing`. Treat `entry_point` as a Python module path rooted at the repo: `agent` may resolve to `agent.py` or `agent/__init__.py`, and `package.agent` may resolve to `package/agent.py` or `package/agent/__init__.py`. Existence of **either** candidate passes; this is deliberately looser than CPython's package-over-module precedence (which would pick `agent/__init__.py` if both exist), because step 4's load check imports under real Python semantics and is the authority on which file actually loads — the static layer only rules out the "named a file that simply isn't there" case. This layer checks existence only; importing it is the step-4 load check's job.

   Both the `manifest.json` read and the entry-point existence probe must resolve **inside the fetched tree root and not follow symlinks out of it** — the static check runs in the _unsandboxed_ backend process over an attacker-influenced git checkout, so a `manifest.json` symlinked to `/etc/passwd` or an `entry_point` whose candidate path escapes the root via a symlink would otherwise be a host-filesystem probe. Resolve each candidate and reject (as `entry_point_missing` / a malformed-manifest reason, not a crash) anything whose real path is not under the tree root, mirroring the harness's own `_ensure_module_loaded_from_repo` out-of-root rejection in [manifest.py](../../harness/src/game_sandbox_harness/manifest.py) so the static and dynamic halves agree on the boundary.
...
6. The manifest's `template_version` matches the open iteration's `deps_version`, else `template_version_mismatch`. This keeps the Stage 5 seed aligned with [leaderboard.md](../../docs/specs/leaderboard.md)'s rule that every submission in an iteration runs on the same dependency set. Note the ordering consequence: because check 5 runs first, in the **real Stage 5 deployment (only v1 exists, iteration `deps_version` is 1)** any `template_version` other than 1 trips `unknown_template_version` and check 6 never fires — `template_version_mismatch` only becomes reachable once a deployment has a base image for a version the open iteration does not pin. The check is implemented now to future-proof multi-version deployments and is proven only by the synthetic unit case below, not by the single-version exit criterion.
...
- `template_version` known to the deployment but different from the open iteration returns `template_version_mismatch`. Because the validator takes the known-version set and the iteration's `deps_version` as explicit arguments, this is exercised with a synthetic set like `{1, 2}`, iteration `deps_version` 1, and a manifest `template_version` of 2 (see line above on why it is unreachable in the real single-version deployment).
