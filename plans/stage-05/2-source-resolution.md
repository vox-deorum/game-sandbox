...
Run `git` through a small process wrapper with an explicit timeout and no credential prompt (non-interactive env, `GIT_TERMINAL_PROMPT=0`), so an unreachable or auth-walled repo fails as a typed error rather than hanging. Stage 3's restricted-import rule currently bans `child_process` across `backend/src`; this step adds one narrow allowlist for the git process wrapper under `backend/src/submission/source/`, and no other backend module may spawn processes. Confine any GitHub HTTP client to this module the same way dockerode and kysely are confined elsewhere.
