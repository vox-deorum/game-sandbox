...
- **Image resolution branches.** Today `start()` always calls `currentSessionBaseImageSpec()` and `driver.ensureImage(...)` for the base image. For a `submissionId` run it instead calls the submission-image helper, which builds an `ImageSpec` of the new `submission-overlay` kind (step 4) for the submission's pinned `deps_version` and id; `ensureImage` honors `DOCKER_IMAGE_POLICY` for reuse-vs-rebuild. The submission must be `ready` for the env's open iteration, else `OrchestratorError(409/400)`.
