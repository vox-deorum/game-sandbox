...
- **Image resolution branches.** Today `start()` always calls `currentSessionBaseImageSpec()` and `driver.ensureImage(...)` for the base image. For a `submissionId` run it instead calls the submission-image helper, which builds an `ImageSpec` of the new `submission-overlay` kind (step 4) for the submission's pinned `deps_version` and id, and `ensureImage` honors `DOCKER_IMAGE_POLICY` for reuse-vs-rebuild. The submission must be `ready` for the env's open iteration, else `OrchestratorError(409/400)`.
- **`sessionConfig` threads the overlay path into the slot binding.** Today every non-human slot is emitted as `{ kind: 'builtin-agent' }` with no path. For a submitted-agent run, `player_0` is emitted as `{ kind: 'builtin-agent', path: '/opt/agents/submissions/player_0' }`. This is the only place the path appears; the harness already accepts it (below).
- **Mode/attribution.** A submitted-agent watch run is recorded with `mode: 'scripted'` (it is a non-human, watch-only run) and the `session_submissions` row is the discriminator that says _which_ agent ran and ties the session to the submission for profile history — no new `SessionMode` value or `sessions`-table column is needed this stage. (The existing allowlist gate on `start()` still applies: launching any container, watch included, requires the session allowlist, exactly as Stage 4 watch runs do.)

The live runner already accepts a path override on a `builtin-agent` slot binding (`SlotBinding("builtin-agent", path=...)`, exercised by the harness's `test_build_slots_builtin_agent_loads_through_manifest`), so Stage 5 can bind `player_0` to `/opt/agents/submissions/player_0` without inventing a new harness slot kind. The scope is deliberately the **current single-agent environment**: Flappy Bird has one slot. The machinery is built so environments with separate human and agent slots can reuse it for play-with-agent sessions later, but **this stage does not pretend there is an opponent slot**. Choosing a submitted opponent for a human-capable slot appears only when an environment actually exposes both human and agent slots, with the first full multi-agent version in Stage 8. Human-controlled Flappy Bird stays a human-controlled single-slot session through the Stage 3 path, untouched.
