...
- **HTTP routes** ([backend/src/app.ts](../../backend/src/app.ts)): `GET /api/submissions/capabilities`, `POST /api/submissions/reachability` (local refused 403 when the gate is off), `POST /api/submissions` (202 + enqueue, never inline; `409 no_open_season`; `409 resubmit_conflict`; identity from the seam, never the body), `GET /api/submissions/:id` (joined with its ordered checks), `GET /api/submissions` (the user's history), `GET /api/environments/:envId/submissions` (active by open season + status). Wired in [main.ts](../../backend/src/main.ts) with the source seam, the worker (overlay-eviction sweep as its `onOverlayBuilt` hook), and startup re-enqueue.
