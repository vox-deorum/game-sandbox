...
- **Validation worker** ([backend/src/submission/worker.ts](../../backend/src/submission/worker.ts)): a bounded (concurrency-1) in-process queue that drives a pending submission through the four ordered, logged stages: `resolve` (source resolve + pin + checkout), `static`, `build` (overlay image), `load` (sandboxed load check). Each transition writes the per-stage check before the rollup; a structured stage failure stops the pipeline and writes the matching terminal status (`static_failed`/`build_failed`/`load_failed`); a `finally` disposes the fetched tree on every path and the crash wrapper closes a thrown stage's `running` check and writes its rollup so nothing is left permanently `running`. `start()` re-enqueues active `pending` rows; the `(submission_id, stage)` upsert makes a re-enqueue a clean fresh run. `whenIdle()` is the test/shutdown sync point. Reads the season's pinned `deps_version` via the new `Storage.getSeason(id)`.
- **HTTP routes** ([backend/src/app.ts](../../backend/src/app.ts)): `GET /api/submissions/capabilities`, `POST /api/submissions/reachability` (local refused 403 when the gate is off), `POST /api/submissions` (202 + enqueue, never inline; `409 no_open_season`; `409 resubmit_conflict`; identity from the seam, never the body), `GET /api/submissions/:id` (owner/operator only, joined with its ordered checks), `GET /api/submissions` (the user's history), and `GET /api/environments/:envId/watch-agents` (viewer-specific ready agents from the play-open season). Wired in [main.ts](../../backend/src/main.ts) with the source seam, the worker (overlay-eviction sweep as its `onOverlayBuilt` hook), and startup re-enqueue.
...
- **Tests:** backend `submission/worker.test.ts` (8) and `submission/api.test.ts` (12): full backend suite 196 pass; frontend `submit-agent-form.test.ts` (9) plus the updated `environment.test.ts` mock: full frontend suite 89 pass. Both `npm run check` gates clean. The build/load leg reuses the step-4 Docker-gated coverage.
...
- A new `Storage.getSeason(id)` was added (index.ts + kysely/) so the worker reads the submission's own season's `deps_version` rather than coupling to the global default: forward-compatible with Stage 6 closing seasons.
...
- **Submit** - given a source for an environment (a git URL plus optional ref, or, when `ALLOW_LOCAL_SUBMISSIONS` is on, a local path), the route resolves the open season (step 1), creates the pending submission row, enqueues the validate-and-build job, and returns `202 Accepted` with the submission id and current `pending` status. Resubmission supersedes the prior active submission when the pending row is created, so even a failed resubmission is the active submission until the owner submits again. The route does not perform git resolution, static validation, image build, or load check inline after the row is written.

  Two refusal cases are handled explicitly rather than crashing or inventing data. If `getOpenSeason` returns nothing for the environment: impossible right after the startup seed, but reachable once Stage 6 can close a season without opening the next: the route refuses with a typed `409 no_open_season` and writes no row, rather than `null`-dereferencing or inventing a season; the form surfaces it as "submissions are closed for this environment." The concurrent-resubmit conflict the storage transaction can raise (step 1's `createSubmission`) surfaces here as a retryable `409` as well, not a 500.

- **Submission read/list** - three reads. Fetch a single submission's status, reason, **and its per-stage validation log** (`listSubmissionChecks` from step 1: the ordered `resolve`/`static`/`build`/`load` checks with their `running`/`passed`/`failed` state and detail). This read is restricted to the submission owner or an operator because anonymous watch-list responses still carry the submission id needed to start a run. List the current user's submissions. Finally, list viewer-specific watch agents for the play-open season. A regular viewer receives only the submission id, anonymous sequence, and `unrated`/`rated`/`own` state. An operator additionally receives owner and source details. The form uses the individual read result, the watch picker uses the redacted watch-agent response, and the agent profile uses the user/history listing plus each submission's log.
...
- **Backend (Vitest, FakeDriver / `:memory:`, no Docker for the non-build paths):**
  ...
  - a submit when the environment has no open season returns `409 no_open_season` and writes no row;
