...
- A new `Storage.getIteration(id)` was added (index.ts + kysely/) so the worker reads the submission's own iteration's `deps_version` rather than coupling to the global default — forward-compatible with Stage 6 closing iterations.
