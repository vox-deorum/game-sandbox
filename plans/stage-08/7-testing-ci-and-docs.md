...
Status: done. The Docker-gated Spades chat integration tests landed in the existing `backend-integration` lane, and the browser journey added a Spades season through the multi-seat scheduler over the `environments/spades/examples/*` roster to a released board where partners share the team score. No new CI lanes: the integration tests join `backend-integration` and the browser journey joins the manually-dispatched `e2e.yml`. All Docker-free suites, the strict docs build, the Docker-gated integration lane, and the browser journey are green.
...
- **Contributor docs:** update the [environments guide](../../docs/contributors/environments/) metadata table if the messaging fields need a row, and the [template guide](../../docs/contributors/environments/templates.md) for the third template.
