...
The override becomes partly editable where operators live: `frontend/src/components/admin/SeasonConfigEditor.vue` provides an enabled toggle following the editor's existing timeout-field pattern. Scripts and the API can set `message_cap`, but console saves omit a stored cap. The LLM block stays preserved and inert until Stage 9. The hands-on demo only needs the console toggle to silence messaging.
...
- The season editor round-trips messaging enablement, while scripts and the API can set `message_cap` and console saves omit it.
