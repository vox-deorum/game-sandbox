...
- A `templates/spades/` layer over the shared `templates/base/`: an `agent.py` stub and `README.md` explaining the `Discrete(66)` encoding (cards 0–51, bids as `52 + k`), the two-phase mask, and the partnership (your partner is the seat across), a thin `sandbox/play.py` adapter for the Spades-specific bid-then-play window and human-seat flow, and the generated `sandbox/env/`. Manifest loading and the standard headless four-seat episode loop delegate to the dependency-free base `sandbox.multiseat_play`. It mirrors the structure of `templates/hearts/`.
