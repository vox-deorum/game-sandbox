...
10. The automated board uses one summary-first `LLM usage` line with successful calls, tokens, and stored budget cost units. A `By model` disclosure provides the breakdown without changing rank.
