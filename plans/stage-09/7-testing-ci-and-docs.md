...
- Two uncached per-slot inflight reads discount successful, retried, and terminal proxy calls from hook, step, and episode timing. A failed or invalid read charges full wall time. An opponent's overlapping request may also advance the cumulative counter and reduce the acting player's chargeable time.
...
- A forced post-upstream accounting failure returns `meter_unavailable` and makes only that pair unavailable until the backend restarts. Rejected requests make no upstream attempt, and another pair remains available.
...
- Docker-free tests, delivered in steps 1 through 6 and extended here where this step finds gaps, cover every retry class, compatible error path, response-metadata sanitization, generic per-scope rate-event retention, completion-limit normalization, ordinary-content tiktoken fallback, reservation release, sticky post-upstream meter and ledger failure handling, successful-only record sink, frozen workflow policy, authorization boundary, fresh flat-schema creation, and UI state.
