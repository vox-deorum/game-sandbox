...
Status: complete. All build-order steps are done, and the stage-03 documents match the implementation. Steps 1, 2, and 3 landed first. Step 1 added the backend package skeleton, the storage layer, and the generated `environments.json`. Step 2 added the driver-neutral execution-driver interface, the `FakeDriver` test double, and the import-isolation lint config. Step 3 added the `Episode` extraction in `session.py` behind the Stage 2 regression gate, the `live`/`live_io` modules, and their pytest suites. The remaining steps followed. Step 4 added the dockerode driver and the `session-base:deps-v1` image, with the memory-quota, no-network, and orphan-reaping driver tests: containers and networks carry a `game-sandbox.owner-pid` label, and at driver construction the driver reaps every item whose labeled pid is dead or equal to the current process id, since construction runs before the process has launched anything of its own and a label carrying its own pid can only belong to a previous incarnation; dockerode's zero-config defaults reach the daemon over the host socket or named pipe, and reach it the same way when that socket is mounted into a containerized backend, so driver construction needs no connection configuration in either case. Step 5 added the orchestrator, the per-session relay, the `protocol` line-classification and command envelopes, and the Fastify HTTP/WebSocket API, all over the `FakeDriver`. Step 6 added the scripted-WebSocket-client, human-slot-timeout, and idle-timeout end-to-end tests as a Docker-gated Vitest project, plus the `backend-integration` CI job and the workspace-wide `check:ts`/`test:ts`. Step 7 added the [backend](../docs/contributors/runtime/backend.md) and [execution](../docs/contributors/runtime/execution.md) contributor pages; the live-runner section lives in the execution page, the natural home for the container side of the transport. The exit criteria below are met and exercised by the integration suite.
