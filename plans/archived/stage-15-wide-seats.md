...
- Spades declaring a partnership plan and a solo plan, the shared card renderer marking wide seats, a companion-or-self human assignment for a wide seat, dynamic chat recipients, and the seat grid and standings detail that follow.
...
- [Interaction](../docs/specs/interaction.md): grid sizing from the resolved seat layout, ordered human-controlled players, and an optional companion agent for the rest of a wide seat.
...
| `human_timeout_ms` | per turn | none, used whenever a human-controlled player acts |
...
- **Seat grid.** One assignment control per seat, with a short hint in the same row giving the number of players the seat covers, such as "2 players". Positions are not listed. Selecting Human on a wide unrestricted seat reveals one required control for the seat's other players. It offers the ordinary builtin and submission choices, plus self-play when every member is human-capable. The user makes that choice explicitly before starting.
...
- **Human play.** A human assignment occupies one seat. With an agent companion, the person controls the first human-capable member in declared order and one selected companion agent is instantiated separately for every other member. With self-play, the person controls every member and no companion is instantiated. Self-play requires every member to be human-capable. Each controlled player's turns use `human_timeout_ms`. One human seat per session still holds under `MAX_HUMAN_SEATS`.
...
- A seat with no human-capable member is never offered to a human. A wide unrestricted human assignment is rejected until it has one legal choice for the other players. An agent choice makes only the first human-capable member external. A self choice is available only when every member is human-capable, makes every member external in declared order, and creates no companion binding.
...
- A one-player session receives exactly today's container memory, and a four-player one receives the base plus three increments. The chargeable workflow watchdog scales with the resolved player count.
...
- A Spades season runs end to end on each plan through the admin console, the projected game counts match the figures above, and the partnership replay list shows a winner rather than "Tied". A human session on each plan shows the seat-grid hint and the explicit companion-or-self choice for a wide seat. A self-played partnership reveals and accepts input for both hands, applies the move clock to each controlled turn, keeps one primary chat sender, records human attribution on both players, and preserves the shared card renderer's wide-seat marking.
