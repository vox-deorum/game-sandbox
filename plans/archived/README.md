...
| [16. Named builtins and a restricted seat](stage-16-named-builtins.md) | Multiple builtins, one Human-or-builtin seat, and matching live and admin UI |
| [17. Simultaneous stepping](stage-17-simultaneous-stepping.md) | Parallel environments, one state per tick, unified asynchronous messaging |
| [18. Canonical zod contract](stage-18-canonical-zod-contract.md) | One authored definition per wire contract, with JSON Schema generated from it |
| [19. Composite action spaces](stage-19-composite-action-spaces.md) | Dict action spaces with per-key masking, for sequential environments |

## Other completed plans

- [Docker edge proxy](docker-edge-proxy.md) covers the Cloudflare-only nginx TLS origin, loopback administration listener, and private Compose relay network.
- [Days at Three Branches art](days-at-three-branches-art/README.md) preserves the retired environment's source and runtime artwork for reference.

Communication follows multi-agent and builds its own test bed: Spades, a partnership environment where targeted partner signals and broadcast warnings genuinely differ, while Hearts stays messaging-free. Documentation, the semantic contract, and the user system can land independently after communication. The LLM proxy and accounting model do not depend on messaging semantics, but the current Stage 9 plan comes last because it uses the semantic Hearts helpers and real account authorization, covers credentials in the existing `chat` hook, and keeps Spades as a disabled-session regression. Environment variants come next to last: they extend the season config and admin console from Stage 6, the multi-seat start flow from Stage 7, and the harness launch path as Stage 13 left it. Wide seats build directly on the reserved-parameter pattern those variants introduced, so they come after: a seat stops being the same thing as a PettingZoo player and may cover several of them, which lets one agent play a whole Spades partnership. Named builtins and simultaneous stepping are the platform work the future role-playing environment needs and are independent of each other. Stage 16 lets an environment ship several builtins and restrict one declared seat to Human or one designated builtin, which each matchup places directly alongside its submission seats. Stage 17 adds parallel environments where every active player decides each tick and gives every environment one asynchronous messaging model. The role-playing environment itself is a later stage consuming both. Stage 18 is platform work rather than a product feature: it collapses each wire contract to a single authored definition in zod and generates the JSON Schema the Python harness validates against, so a type and its validator can no longer disagree. Stage 19 is platform work too, and widens the Stage 11 action contract: an environment may declare a Dict action space whose legal moves are masked key by key, which lets a game express one action as several independent choices rather than encoding them into a single integer. It is sequential-only until a PettingZoo release can sample an object mask on the parallel path.
