...
Part of [Stage 4](../stage-04-frontend-core.md). The exit criteria are executable, split by what they need:

- Frontend unit tests in jsdom, with no canvas and no network.
- Backend unit tests for the allowlist and retention, on the existing in-memory setup.
- A browser end-to-end suite for the criteria that only mean something with a real renderer over a real session.

## Frontend unit tests (Vitest, jsdom, no backend)

jsdom implements no WebGL or canvas rasterization, and pulling in a native GPU/canvas package just for tests is a Windows-CI tax not worth paying. The renderer's split of a pure `computeScene` from the PixiJS reconciler (see [flappy-bird-renderer.md](flappy-bird-renderer.md) and [rendering.md](../../docs/contributors/environments/rendering.md)) exists partly for this: everything with logic in it is testable as plain functions, and actual pixels are the end-to-end suite's job. The project moved to Vue, so the component-shaped suites use `@testing-library/vue`, not React. Suites:

- **Scene computation**: states from a checked-in Stage 2 determinism fixture produce scenes with the bird, every pipe, and the HUD values in the right places. The suite also checks the purity property: states fed in shuffled order yield scenes identical to in-order, which is what the scrubber relies on.
