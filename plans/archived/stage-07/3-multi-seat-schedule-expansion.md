...
The default competitive expansion still enumerates distinct `P(N, K)` or `C(N, K)` seatings for fair head-to-head coverage, so the worked counts above are unchanged. What the scheduler guarantees is that a repeated agent ref in a `slots` assignment is emitted, resolved, and run without dedup or error; the Naive baseline already relies on this (one ref in every submission seat). There is, however, no season-config surface today that requests a genuine self-play or mirror-match shape (a submission seated against copies of itself): the config's `matches` declare seat specs, not concrete repeated seatings, and the default rotation never produces one. So self-play is a capability the scheduling primitive supports rather than a shape a season can currently ask for; adding it would be a config extension, not a scheduler change.
