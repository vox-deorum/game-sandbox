...
The live orchestrator and automated workflow runner use the same composition helper. Identical seatings share one in-flight build and acquire references to the complete image. The driver synchronizes acquisition, final release, and eviction by composition; active images and build intermediates cannot be removed. Every caller releases its acquisition after teardown or a failed launch. Source-tree disposal attempts every tree and releases an acquired image if a disposal failure prevents returning it. Tests cover concurrent repeated seatings, release and eviction races, failed-build retry, and cleanup failures.
