...
The environment page gains a play-season section directly above the play and rate section, shown while a season is open for play: "Open for Play" followed by the season name as its heading, the season's description markdown, and a compact settings info box using the `UiCard` info variant. The variant has a stronger solid border and raised-surface background and appears on `/styleguide`. The box has no heading or prefix. It lists differences from the environment defaults, pairing each default with the season value. A season with no differences says "This season uses the default settings" inside the same box. The released leaderboard section uses the same box for its selected season. When the play-open and released season are the same, the page shows the box once; distinct seasons receive separate, contextually labelled boxes. The play-season section owns the "Play" action. The section below it is headed "Play and Rate" followed by the same season name, so the listed agents are plainly the ones playable and ratable in that season. The play dialog title names the season too. When play is closed, the empty-state copy says that no season is currently open for play. The multi-seat decision reads the resolved seat count instead of `meta.max_slots`. The play-season section and the boolean-as-select control are approved visual patterns.

My Submissions uses the same settings info box for the submission-open season. A plain metadata line identifies the season and active submission status, with the danger badge reserved for failure. Its form begins with **Public Repository URL**, whose hint explains the verification, validation, and build sequence, then the optional source revision and rating-prompt fields. **Set Up Locally** is inside the form action row immediately before **Verify Reachability**.

The selected-season Leaderboards header uses plain release and board-count metadata followed by the same settings info box from the server-resolved settings returned with the boards. Only an unreleased operator preview uses a semantic badge. The **All Seasons** history remains a clean index without a repeated settings box.
...
Open for Play: Spring 2026                        [Play]
+----------------------------------------+
| This season uses the default settings. |
+----------------------------------------+
```
