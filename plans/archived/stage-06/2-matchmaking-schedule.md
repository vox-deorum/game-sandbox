...
- **Single submission seat (the Flappy Bird case).** Consider a match with one `submission` seat, where the rest are `builtin-naive` (possibly none). For each active `ready` submission, fill the submission seat with that submission's `AgentRef`, fill every built-in seat with `{ kind: 'builtin-naive' }`, and emit `games` total runs for that resolved assignment. Seeds round-robin by run index: run `i` uses `seeds[i % seeds.length]`. With two seeds and `games: 2`, each resolved assignment runs twice, once per seed. With two seeds and `games: 5`, the seeds are `seed0, seed1, seed0, seed1, seed0`. When a match's `seeds` list is empty, the scheduler draws `games` fresh random seeds for that match once at trigger time (through the injectable `randomSeed` generator on `BuildScheduleInput`, defaulting to the shared `drawSeed` helper) and then cycles through that drawn list by run index exactly as it would an explicit list, so every seating and the Naive baseline within the match share the same seeds.
- **The Naive baseline always runs.** Besides filling empty seats, the scheduler emits the same match with the Naive baseline in the submission seat. This way the board always has a baseline row to compare against (the "Yes, always include Naive" decision). The baseline games play the same seeds as the submissions, whether that seed list is explicit or drawn once per match at trigger time, so its column is comparable.
...
Determinism holds two separate guarantees. Enumeration order and counts are always deterministic: submission refs are sorted by stable submission id before expansion, baseline rows are emitted after the submitted rows for each match configuration, and the same inputs always yield the same ordered schedule with the same number of games, whether `seeds` is explicit or empty. Seed values, though, are reproducible only for an explicit list: seed-cycling is index-driven (`seeds[i % seeds.length]`), so a re-run with an explicit list reproduces a deterministic agent's games exactly. An empty seed list draws `games` fresh random seeds for that match once at trigger time (through the injectable `randomSeed` generator, defaulting to the shared `drawSeed` helper), so a re-run schedules the identical games but with newly drawn seed values.

The function handles these edge cases explicitly, and tests pin each one. Zero ready submissions: the schedule is just the Naive baseline games, so an operator can dry-run a season before anyone submits. A match with no submission seat at all: pure baseline, emitted once for its `games` count. An empty `seeds` list: valid. The scheduler draws `games` fresh random seeds once for the match at trigger time and cycles through them by run index the same way an explicit list would, and those values are persisted with the run. `games` of zero or less: typed rejection. A match with zero slots: typed rejection, also rejected by the admin config validator. An empty overall match list: returns an empty schedule that the trigger refuses with `empty_schedule`, because an unconfigured season can be stored but cannot be run. More submission seats than ready submissions (`N < K`): no submitted seatings, just the Naive baseline, so a thin season still dry-runs rather than erroring.
...
- The Naive baseline is always present, even with zero ready submissions, and the baseline games use the same seed source and count as the submissions.
...
- Guard cases: zero submissions, no submission seat, `games: 0`, zero slots, an empty match list, and `N < K` (fewer ready submissions than submission seats). Each produces the specified schedule or typed rejection. An empty seed list is not a guard case: it is the random-draw mode, covered above. The empty match list case proves the scheduler stays pure while the trigger owns the `empty_schedule` refusal.

## Done when

`buildSchedule` takes a match design and a trigger-time ready-submission snapshot, plus the environment's `seat_order_matters` capability. It deterministically expands them into the ordered, fully-resolved game list that the runner executes and the storage stores. The expansion follows the exact game-count and seed semantics, fills each match's `submission` seats by the seat-order-aware rotation (single-seat being the `K = 1` case), and always includes the Naive baseline for runnable submission-seat designs. Zero-slot matches and non-positive game counts fail with typed errors for the admin API to surface, while an empty seed list is valid and draws `games` fresh random seeds for that match once at trigger time, cycled the same way an explicit list would be. No Docker, DB, or route work is part of this slice. It is the pure input to step 3's trigger and step 4's runner.
