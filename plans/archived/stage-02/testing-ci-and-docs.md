...
The contributor environment guide is organized under `contributors/environments/` by package, rendering, and template-and-example topics. Its index provides the add-an-environment checklist and links to each topic. The `environments/README.md` placeholder is replaced with a pointer to that guide, and the harness docs page gains the session-loop and CLI sections.
