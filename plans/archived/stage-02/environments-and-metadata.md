...
`EnvironmentMeta` is the serializable public-facing layer, field-for-field what [environment.md](../../docs/specs/environment.md) lists: `env_id`, `display_name`, `description`, `min_slots`, `max_slots`, `human_slots` (tuple of slot ids), typed gameplay `parameters`, `human_timeout_ms` (None when the pace interval governs, see below), `recommended_episode_ticks`, `pace_interval_ms` (None for turn-based), an optional `view_interval_ms` (a watch/replay playback cadence independent of the stepping model, so a turn-based game can be watched at a paced speed without becoming realtime), an optional `live_interval_ms` (a live human-session cadence that spaces out other seats' streamed moves without changing environment stepping or scoring), `step_limit_ms`, `episode_limit_ms`, `messaging` plus `message_cap`, `llm`, `seat_order_matters` for multi-seat scheduler rotation, and `renderer` (a string id the frontend resolves in Stage 4). `EnvironmentMeta.to_json()` emits the snake_case dict the backend serves verbatim and prepends the synthesized `seats` parameter derived from the slot bounds.

`EnvironmentEntry` is the full registration: `meta`, `make(parameters)` (a factory receiving the complete resolved parameter map and returning a fresh AEC env; the seed arrives at reset), `default_action(env, slot_id)` (the environment-provided legal default the loop applies on every timeout path), and an optional `overlay(env)` hook returning the per-step overlay dict. The non-serializable hooks live here, outside `EnvironmentMeta`, which stays pure data. Flappy Bird declares `pipe_gap`; Hearts and Spades accept the required map without declaring gameplay-specific parameters.
