...
Do not list member player ids in the assignment row. Emit the selected values as a `seats` object keyed by the layout's exact seat ids. A seat offers the human option when at least one member is listed in `human_players`; the backend and browser both derive the first capable member in declared seat order. Selecting Human for a wide unrestricted seat reveals `Seat N's other players`, with `Play them yourself` when every member is human-capable plus the same Naive and submission choices as an ordinary agent assignment. Default to `Play them yourself` when it is legal, while a mixed-capability seat still requires an explicit agent companion. Restricted wide seats keep their fixed companion behavior, singleton human seats omit the field, and the single-human-seat-per-session limit remains unchanged.
...
Expose the current messaging contract for an external turn in step state as optional `chat_options` with `sender`, `target_recipients`, and `default_recipient`. The state's `tick` dates the turn but does not identify it on its own: the live-only opening frame and the first recorded step both carry tick 0, so two opportunities can be published on the same tick. The sender and the tick together name one opportunity, and the browser keys on that pair. Extend `schema/step-state.schema.json`, the generated state types, the shared command protocol, and the Python live-command parser together. Adding one optional object needs no `schema_version` bump. A human chat command carries the sender and the tick it was composed against, both required. `frontend/src/pages/SessionPage.vue` enables the composer only when `chat_options.sender` is the designated human player and the state is nonterminal. `frontend/src/composables/useLiveChat.ts` carries those choices and the tick they were published on as one opportunity value, which both pages bind to the panel as a unit. `ChatPanel` renders exactly the allowed direct recipients plus `Everyone`, resets its selection to the new default when the opportunity changes, and sends the sender and that tick with the message.
...
`frontend/src/components/admin/SeasonConfigEditor.vue` receives that count as a prop and owns the projection itself. It recomputes whenever the environment, any layout-affecting parameter including `players` or `seat_plan`, a seat spec, or a games value changes. It reads nothing else in the editor, so an invalid field in an unrelated block such as an LLM limit never blanks it. Display the exact total near the match controls, beside the resolved seat and roster sizes, and each match's own share in that match's heading. The total disappears only when no seat layout resolves, which happens while the environment metadata is still loading or a parameter is invalid. Every other failure is a typed projection error, including a stale match seat count and an invalid game count, and renders its reason in the total's place. The wording labels eligibility as of page load and explains that triggering freezes a fresh roster. Nothing here creates a run, freezes a snapshot, or stages a submission, because the draft never leaves the browser.
