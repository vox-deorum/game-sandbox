...
The outbound stream from the container is JSONL, and recording lines pass through unchanged so the wire form and the stored form stay one format. Three semantic categories exist:

- **Recording lines**: the header first, then one completed-step state per line, exactly as `FolderRecordingStore` serializes them, schema-validated in the harness before emission.
- **Live-only state**: a turn-based session may emit one opening presentation state after the header and before the first completed step. It uses the state schema but is never written to the recording.
...
The shape classification rule is that header and state lines never carry a top-level `kind` property, while event envelopes do. It holds because the state schema defines no such field, and a test asserts that against the packaged schema so the rule cannot rot silently. The same rule classifies WebSocket frames.
...
- The recording header and completed-step states, plus the optional live-only opening presentation state.
...
- **Stdout hygiene.** Protocol lines must own stdout. The first thing `live.py` does is `os.dup()` the real stdout for the protocol writer and `os.dup2()` stderr over fd 1, before any game import. Stray prints from environment internals then land in the diagnostics stream instead of corrupting the protocol.
...
- **`PausableClock`** wraps any `Clock` and subtracts accumulated paused time from `now_ms()`, freezing wall-clock timing and cadence. The official LLM calling-thread CPU floor uses a separate clock and remains chargeable during a pause. Headless runs never construct a live loop, so they never pace and never pause.
...
- **The tee store** mirrors every serialized recording line onto the protocol stream. Rather than a separate re-serializing wrapper, `FolderRecordingStore` grew an optional keyword-only `on_line` mirror hook. Its writer serializes each header and completed-step state once, writes it to the recording file on the mounted volume and flushes, then hands the same string to `on_line`. `build_tee_store(root, protocol)` wires that hook to the protocol's `emit_raw`, so those streamed and stored bytes are identical by construction. A turn-based runner may also emit one live-only opening presentation state after the header without passing it through the store. The live tests assert byte parity for recording lines and verify that the opening state appears only in the stream before the first completed step. Recording directories and files are chmodded after creation, so a root-owned, cap-dropped session container can leave files that the backend user can read and delete on the host bind mount. The default (`on_line=None`) leaves serialization and readback behavior unchanged, so the Stage 2 recording suites are untouched.
