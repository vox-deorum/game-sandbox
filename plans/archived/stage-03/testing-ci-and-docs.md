...
There are two contributor pages. `contributors/runtime/backend.md` covers package layout, config, storage, the identity stub, and how to run the backend locally. `contributors/runtime/execution.md` covers the driver interface and sandbox profile, the transport envelope and line-classification rule, the WebSocket protocol, and how to add a driver: the Kubernetes driver's future landing page. The live-runner section (pacing, pause, the transport source) lives inside `contributors/runtime/execution.md`, because the live runner is the container side of the same execution boundary, so the two halves read together there. `contributors/testing/index.md` covers the `backend-integration` job and the workspace-wide `typescript` job. Student pages are untouched: nothing participant-facing changes until submissions arrive in Stage 5.
