...
Shared card code is split by responsibility. A standard-library-only `environments/local_play/card_utils.py` owns the integer codec, suit and rank accessors, and display names. The pure Hearts and Spades rules engines import from it without gaining NumPy or Gymnasium dependencies. `environments/local_play/card_spaces.py` imports that codec and declares `CARD`, `HAND`, and `TRICK`. `environments/local_play/shared_modules.py` resolves these helpers in repository and composed-template layouts. All three modules sync into every template. Each card-game template also includes its game-specific `sandbox/cards.py` helper module, which imports the shared codec.
...
1. **[Foundation and compatibility](stage-11/1-foundation-and-compatibility.md).** Add the dependency-free card codec and shared Gymnasium spaces, and change the timeout hook while retaining sentinel aliases for direct callers.
