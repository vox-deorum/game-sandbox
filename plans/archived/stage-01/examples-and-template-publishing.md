...
The template source and all example sources live in this monorepo. The student starter kit is composed from an env-agnostic `templates/base/` layer and the hand-authored `environments/<env>/template/` layer colocated with each environment. The design grows in Stage 2 (see [stage-02/template-and-examples.md](../stage-02/template-and-examples.md)). A complete template for an environment is `base` plus its env layer, composed. Each example under `environments/<env>/examples/<name>/` contains only the files that differ from its composed template: typically the agent implementation, a README, and optionally a `requirements.extra.txt` with extra pinned dependencies. An example is therefore a small, reviewable diff, not a second copy that can rot.

Every environment explicitly declares a `PUBLISHED_EXAMPLES` tuple. It selects the source examples that may be published, and may be empty. The declaration names distinct immediate `examples/<name>` directories. Discovery requires each name to start with a lowercase letter or digit, contain only lowercase letters, digits, dots, underscores, and hyphens, exclude `..`, and end in neither a dot nor `.lock`. This selection does not change source discovery: all checked-in examples are composed and tested by examples CI.

## How a template or example becomes runnable

`scripts/compose.py <env>` copies `templates/base/**` into the gitignored `build/templates/<env>/`, generates `sandbox/env/`, `sandbox/harness/`, and the shared card helpers there, then copies `environments/<env>/template/**` on top with whole-file replacement, so an env-layer file always wins. `scripts/compose.py <env> <name>` composes that template, then overlays `environments/<env>/examples/<name>/**` on top the same way into `build/examples/<env>/<name>/`. There is no manifest. The convention plus one merge rule is the whole mechanism.
...
- Each environment's composed template is force-pushed to a `templates/<env>` branch, and each example selected by `PUBLISHED_EXAMPLES` to an `examples/<env>/<name>` branch.
...
The publish workflow does not contain composition logic of its own. Its jobs are thin wrappers around the same scripts developers and CI use. `scripts/compose.py` builds every source template and example for local checks and CI, while `scripts/publish_template.py` selects only published examples for content replacement, commits, tags, and pushes. Once all desired branches have pushed, a real publish enumerates only `refs/heads/examples/` and deletes stale generated example branches. Local development, CI verification, and publishing all exercise one code path, so the artifact a student clones is byte-identical to the one CI tested, and the whole pipeline can be run and debugged locally given a token. A dry run does not contact or mutate the student repository. That is how the tag-to-publish path is rehearsed locally (see [testing-and-ci.md](testing-and-ci.md)).
