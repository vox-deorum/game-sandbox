...
The server is authoritative. It steps the environment at the environment's fixed tick rate (part of its [metadata](environment.md)) and pushes one state object per tick to connected renderers over the session's WebSocket. Renderers send human inputs as they happen, tagged with the slot they control. On each tick, the environment takes the latest input received for each human-controlled slot, or a noop if none arrived. The browser never simulates ahead.
...
Turn-based environments have no tick rate. The server steps them as actions arrive instead: an agent slot is stepped as soon as its agent returns an action, and a human-controlled slot waits until the browser controlling that slot submits a move. Everything else about the loop is unchanged. The server stays authoritative, one state object goes out per step, and the browser never simulates ahead.
...
Human-controlled slots use a separate timeout from agent decision timeouts. The environment provides a default value, and a session can override it. In realtime environments, this timeout controls when the server substitutes a noop for a slot that has not sent input recently. In turn-based environments, it controls how long the slot may wait before the harness applies an environment-provided legal default action, such as the lowest legal card in a card game. Renderers should show the active timeout when it affects play, for example as a move clock.
