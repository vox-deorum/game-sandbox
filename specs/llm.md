...
Agent code reads `OPENAI_BASE_URL` and `OPENAI_API_KEY` from its environment, so the stock `openai` Python client works with no configuration. During local development, the template repo instructs participants to put the class-provided key in a `.env` file (see [submission.md](submission.md)). Server-side, the harness swaps those values for the gateway's address and a dynamically generated one-off key scoped to the session; the key authorizes nothing but that session's gateway access and is revoked when the container exits. The agent code is identical in both places, it just reads the environment.
