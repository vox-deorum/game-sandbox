...
Run `npm run sync:envs` from the repository root after adding or changing an environment. The command discovers package directories here, excluding patterns in `.envignore`, and regenerates Python entry points, wheel packages, and backend metadata. Shared packages such as `local_play/` remain in the wheel but are excluded from environment discovery. Renderer, test, template, and example directories are excluded from the wheel.
