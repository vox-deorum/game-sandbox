...
A targeted message to your partner is structurally different from a broadcast to the table, and Spades' across-the-table partnership is what makes that distinction real. Everything is read through the provided `sandbox.cards` helpers, so the agent never decodes the combined `Discrete(66)` action space by hand. Your partner is the player across, `player_((your_player + 2) % 4)`; every message is recorded and shown in replays.
