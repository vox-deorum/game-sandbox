...
Motion only describes a change: glides, short lunges, pale-bone arrows, rising damage, ink dissolves, and brief capture blooms. A transition uses the full 1000 ms host cadence. Activation holds from 0 to 150 ms. The actor follows every tile in its exact executed path, with movement ending at 500, 583, 667, or 750 ms for one through four tiles, then settles until resolution starts at 650, 683, 717, or 750 ms. Resolution merges attack and reaction: attack begins at resolution, a targeted reaction joins one quarter of the way through, and a capture-only reaction begins immediately. All finish at the atomic final-state commit at 1000 ms. Damage numerals use an opaque black outline that stays two CSS pixels wide across display scales. The preceding units and HUD remain on screen until the timeline completes. The event actor keeps its gilt seal and movement range through settle, then the range clears for resolution. A defeated target stays intact until reaction begins. Seeking, mounting, resizing, and repeated ticks show the completed frame directly. Event motion is the replay's content, so the OS reduced-motion preference does not still it; remote desktop sessions report that preference unconditionally. The schedule itself lives in `timeline.ts`, the presentation thresholds and terrain marks in `presentation.ts`, and the palette in `scene.ts`.
