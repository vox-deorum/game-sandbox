...
Crane Reach is an aged parchment field set in a greener night-ink surround. The palette is parchment `#cfc5a9`, bone `#efe7d3`, dilute grid ink `#6f6757`, reed `#a9ae8a`, silt `#bfa072`, slack water `#5a7680`, cinnabar `#b0402e`, indigo `#3a5f8f`, gilt `#d9a441`, and mulberry `#7d5a7e`.
...
Terrain always combines wash and mark. Grassland is a muted bare reed wash with no paired grass tufts. Hills use clearly separated silt contours, water uses broad slack ripples, forest uses canopy, and marsh uses one or two sedge tufts. Waste is asset-only artwork for a magic-polluted map feature, not a terrain type or gameplay rule. Marks need enough contrast and stroke weight to be understood at token scale.
