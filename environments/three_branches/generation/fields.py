"""Elevation and moisture noise, sampled once per cell before anything is painted.

Both fields are pure-Python fractal value noise: a lattice of stream draws covering the frame plus
one ring of nodes, read back through smoothstep-faded bilinear interpolation and normalised to the
unit range. Elevation then takes the southward slope bias, so water has somewhere to flow. The
fields are generation-only artifacts and never reach the layout.
"""
