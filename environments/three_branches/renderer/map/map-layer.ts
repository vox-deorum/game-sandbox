...
import { fillTintHex, HEARTHSIDE_STYLE, THREE_BRANCHES_PRESENTATION } from '../core/presentation.js'
import type {
  ContourCoordinate,
  StaticScene,
...
function routeFadeLayers(
  ...
  for (let step = 0; step < ROUTE_EDGE_FADE_STEPS; step += 1) {
    ...
    const extraWidthCells = 2 * edgeFadeCells * (1 - (2 * step) / (ROUTE_EDGE_FADE_STEPS - 1))
  ...
  for (let index = 1; index < routes.roadGuide.length; index += 1) {
    ...
    target
      ...
      .stroke(
        routeStroke(
          fill,
          (Math.min(previous.widthCells, point.widthCells) + extraWidthCells) * cellSize,
        ),
      )
  }
  for (const point of routes.roadGuide) {
    target
      .circle(
        point.x * cellSize,
        point.y * cellSize,
        ((point.widthCells + extraWidthCells) * cellSize) / 2,
      )
      .fill(fill)
  }
}
