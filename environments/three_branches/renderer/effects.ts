...
import { HEARTHSIDE_STYLE, type HearthsidePaletteKey } from './presentation.js'
...
export function emissiveSpec(type: string, state: string): EmissiveSpec | null {
  ...
  return { frame: HEARTHSIDE_STYLE.emissives.frame, tint: HEARTHSIDE_STYLE.emissives[type], alpha: type === 'lantern' ? 0.48 : 0.56, scale: type === 'lantern' ? 1.1 : 1.3 }
}

function activeEffect(type: string, state: string): keyof typeof HEARTHSIDE_STYLE.propEffects | null {
...
function requiredFrames(frames: readonly string[] | undefined): readonly string[] {
  if (frames === undefined || frames.length === 0) throw new Error('Three Branches prop effect frames are missing.')
  return frames
}
