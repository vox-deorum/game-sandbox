...
describe('Three Branches prop effects', () => {
  ...
  it('changes with fractional presentation tick and uses the configured palette tints', () => {
    const active = [['lantern', 'lit', 'gilt'], ['hearth', 'lit', 'cinnabar'], ['shrine', 'tended', 'violet'], ['pump', 'flowing', 'water'], ['bell', 'ringing', 'gilt']] as const
    for (const [type, state, tint] of active) expect(propEffectSpec(type, state, type + ':one', 3)?.tint).toBe(tint)
    expect(propEffectSpec('hearth', 'lit', 'hearth:one', 3.1)).not.toEqual(propEffectSpec('hearth', 'lit', 'hearth:one', 3.4))
  })
  ...
  it('animates only the five configured active states', () => {
    ...
    for (const [type, state] of [['lantern', 'lit'], ['hearth', 'lit'], ['shrine', 'tended'], ['pump', 'flowing'], ['bell', 'ringing']] as const) {
