...
def leaderboard_scores(state: SpadesState) -> list[int]:
    ...
    fared), mirroring the raw negated-penalty precedent in ``hearts.rules``. Team scores reach into
    the hundreds in both directions (the floor is minus 260, both partners bidding 13), so these are
    int16-range values, not the int8 that suffices for Hearts penalties.
    """
...
def display_scores(state: SpadesState) -> list[int]:
    ...
    surfaced per seat so the local standings and any per-seat display can index it by seat.
    """
