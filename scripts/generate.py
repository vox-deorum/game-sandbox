...
def sync_template_env() -> None:
    """Copy the import-self-contained environment modules into the template's sandbox_env/.

    Only the modules that import nothing of ours (relative + third-party imports only) are
    copied verbatim; the harness-dependent flappy_bird/__init__.py is replaced by a minimal
    generated __init__. The staleness CI job diffs this location like every other generated
    output, so the template copy is provably current on every PR.
    """
    if TEMPLATE_SANDBOX_ENV.exists():
        shutil.rmtree(TEMPLATE_SANDBOX_ENV)
    (TEMPLATE_SANDBOX_ENV / "flappy_bird").mkdir(parents=True)

    for relative in SYNCED_ENV_MODULES:
        shutil.copyfile(ENVIRONMENTS_SRC / relative, TEMPLATE_SANDBOX_ENV / relative)

    (TEMPLATE_SANDBOX_ENV / "__init__.py").write_text(
        _SANDBOX_ENV_INIT, encoding="utf-8", newline="\n"
    )
    (TEMPLATE_SANDBOX_ENV / "flappy_bird" / "__init__.py").write_text(
        _SANDBOX_FLAPPY_INIT, encoding="utf-8", newline="\n"
    )
    print(f"  template sandbox_env -> {TEMPLATE_SANDBOX_ENV}")
