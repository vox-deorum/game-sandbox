...
# surface — make_env plus ENV_ID, PLAYER_SLOT, and make_human_controller — so the env-agnostic
...
def _render_sandbox_init(env_id: str, spec: _TemplateEnvInit) -> str:
    ...
    surface_import = (
        ...
        f"{spec.default_action}, extract_overlay, make_env, make_human_controller"
    )
    return f'''\
...
__all__ = ["make_env", "make_human_controller", "extract_overlay", "{spec.default_action}"]
'''
...
def sync_template_env() -> None:
    ...
    for env, modules in TEMPLATE_ENVS.items():
        ...
        inits = {
            "__init__.py": _render_sandbox_init(env, spec),
