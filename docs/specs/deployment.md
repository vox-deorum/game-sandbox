...
- **A single-tenant machine.** The mounted daemon socket grants full control of the daemon, which is root-equivalent on the host. The machine hosting a containerized deployment runs nothing that must be protected from this app. The backend drops to a non-root account at startup and the socket mount is read-only, but that is hardening rather than containment: a compromised backend keeps the daemon's full authority.
