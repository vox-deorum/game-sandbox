...
The browser state is an interface hint, not the authority. The harness accepts a human message when its sender matches the current external actor and its tick names either the current opportunity or the immediately preceding opportunity that may have raced its drain. That one-turn grace expires at the next drain. The harness applies the recipient policy published for the named opportunity and drops an older or disallowed message. It drains only the acting human player's bounded first-in, first-out queue, so a message cannot wait under an inactive identity and appear on an unrelated turn. A valid message travels with the turn that drains it. See [Communication](communication.md).
