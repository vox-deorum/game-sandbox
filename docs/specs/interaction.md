...
A still picture has more than one cause, and a viewer cannot act on it without knowing which. The interface therefore names the reason beside the session status: that the connection dropped, that nothing has arrived for some seconds, that the agents are thinking (naming the player responsible when one dominates the tick), or that the time went to the server rather than to any agent's own charged work. Silence and delivery checks use the environment's actual `pace_interval_ms`, not a playback view cadence. For an unpaced turn-based session containing human players, the browser has no generic authoritative next-turn indicator, so it suppresses silence and delivery warnings. It still reports measured agent and server work, with human-turn waiting excluded from server step cost. A paused or finished session keeps its own wording instead. Health tracking resets on automatic physical reconnect and on pause or the initial Start gate, resumes with a fresh baseline, and stops when a terminal envelope arrives even if buffered animation remains.
