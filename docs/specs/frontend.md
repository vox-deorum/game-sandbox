...
Navigation is two tiers. A persistent, collapsible **left sidebar** carries the cross-environment sections: **Environments** (the home gallery), **Seasons** (the cross-environment competition view), **Documentation**, and **My Agents** (the signed-in user's agents across environments), with the account block (**My Profile**, and a log-out affordance once OAuth lands) at the bottom. Inside an environment, a **contextual tab strip** carries that environment's tasks: **Overview**, **Leaderboards**, **My Submissions**, an operator-only **Manage** tab, and a season switcher defaulting to the current released season. My Submissions always targets the signed-in user's agent profile for that environment, which contains the submit and resubmit form.
