...
While play is open, the overview names that season, shows its description, and includes a compact settings info box. The box lists only differences from the environment defaults: visible gameplay parameters, decision and game limits, messaging availability and length when supported, and LLM API availability when supported. Each item pairs the default with the season value. When nothing differs, the box says **This season uses the default settings.** The overview also makes the season's play, watch, and rating choices available. When play is closed, the page says that no season is open for play. See [Seasons](seasons.md#public-gates) for the gate rules.
...
For the submission-open season, My Submissions shows the same settings info box as the environment overview. **Set Up Locally** is inside the form action row immediately before **Verify Reachability**. It provides copyable setup commands that clone the season's template branch into a folder named for the environment and season, rename the branch to `main`, and remove the template remote, so copies of different seasons never collide and the student's first push publishes the copy to their own GitHub account instead of failing against the template. It also downloads `season.json` when the season has locally reproducible gameplay or time-limit changes. The setup dialog links to Getting Started and explains where to put the file. The action remains available when the season uses all defaults because the setup commands are still useful.
