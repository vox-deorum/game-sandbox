...
In an official session, chargeable time for `act`, `chat`, and `learn` excludes verified time in the backend proxy for that player, including retry waits. Hook-thread CPU time remains chargeable. A valid post-hook proxy reading becomes the next hook's baseline, which keeps the steady-state path to one synchronous read per hook and means a request still in flight between two hooks is discounted from the later one. If a required reading is unavailable or invalid, the full hook time is charged.

Local agent computation must run on the hook thread. A normal model call also runs there and blocks the hook until it finishes. The template's `sandbox.llm.BackgroundLLM` helper may instead keep one plain-text request in flight on a thread the helper owns, allowing the agent to collect the result from a later hook or tick. The helper captures the active player's base URL and key inside the calling hook. Students do not create threads, and the harness provides no submit, poll, or scheduling API.

The hook discount reads total verified proxy time, including a request marked as background. Because a background request does not block the hook, that discount can reduce the hook only to its hook-thread CPU floor. The synchronous path keeps its existing charging behavior.

Live-session and leaderboard-run timeouts exclude only verified blocking proxy time after they start. A background-marked request never extends either watchdog. Each blocking request's allowance is bounded, so a stuck request or long provider-directed retry wait cannot extend the deadline indefinitely. Idle timeout always uses wall-clock time.
