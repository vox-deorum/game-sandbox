...
An environment whose legal actions depend on state publishes a top-level binary **`action_mask`** beside the agent observation. It wraps the meaningful state as `{"observation": {…}, "action_mask": …}`, which places the mask where PettingZoo's masked sampling expects it. The mask counts positions from the action space's `start`, so position `i` covers the action `start + i`, which for the usual space starting at zero is simply the action `i`. The mask is authoritative for agent legality. Human controls must offer the same legal choices, either listed in the semantic render overlay or derived by the renderer from the overlay's game state. Hearts, Spades, and Skirmish at Crane Reach provide both the mask and the overlay. Flappy Bird needs neither because idle and flap are always legal while its agent is active.

Most current environments use a flat integer action accepted by a `Discrete` space. `env.step()` validates every action and rejects illegal values. Template helpers convert a meaningful choice, such as a card or bid, into the integer accepted by those environments. Skirmish at Crane Reach uses a factorized `Dict` action with independent `path` and `target` choices.

### Composite actions

An environment may declare a `Dict` action space when one action is genuinely several independent choices. Skirmish at Crane Reach is the shipped example: each order combines an independently legal movement `path` with an independently legal attack `target`. An action is then an object with one value per declared key, and the `action_mask` is an object carrying one entry per declared key. An entry of `null` marks that component as unrestricted for the turn. A `Dict` child may itself be one of a small set of permitted types, each with its own mask entry shape:
...
A simultaneous environment may use a `Dict` action space only when it publishes no `action_mask` and every value contained by the action space is legal in every reachable state. In that case PettingZoo samples through the action space itself. A `Dict` action space with a published mask remains limited to sequential environments because the pinned `parallel_api_test` reduces the object mask with `np.flatnonzero`. The correction is merged upstream and has not reached a release.
