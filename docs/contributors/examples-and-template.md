...
Compose also ships each environment's student page inside the template as `environment.md`, so the game's observation and action reference lives in one source (the docs page) rather than being duplicated into the template's `README.md`. It copies `docs/students/environments/<env>.md` (the env id with underscores turned into hyphens) into the composed template and rewrites the page's cross-doc Markdown links to `{{DOCS_URL}}` tokens, leaving in-page anchors and external links untouched. Compose fails loudly if an environment has no docs page.
