...
3. Run `npm run sync:envs`. Directory discovery builds the environment's template specification from its metadata and direct modules, then regenerates `templates/<env>/sandbox/env/`. Keep credited source and license files directly inside the environment package so they are included. Do not add Python human controllers.
4. Add at least one example under `examples/<env>/<name>/`, reading the observation through the helper module so it models the intended style.
5. Write the student documentation page `docs/students/environments/<env>.md` and add its row to the environments index. Compose ships this page inside the template as `environment.md`, the single source for the game's reference, so the template's `README.md` and `agent.py` point at it rather than restating it. The [student-facing deliverables](environments.md#student-facing-deliverables) section lists the required page sections, the helper placement rules, and the template docstring and README standards.
6. Run `uv run python scripts/ci.py all`. This composes and tests every template and example, builds the documentation, checks generated files, and stages a publish dry run without pushing anything.
7. After the change is merged to `main`, dispatch the **Publish Template** workflow with `dry_run: true`. Use the current `N` to republish against the existing dependency snapshot, or the next `N` when the dependency set or template contract changed. Resolve any failure before publishing.
8. Dispatch the workflow again with `dry_run: false`. It publishes the default environment to the student repository's `main` branch, every environment to `templates/<env>`, and every worked example to `examples/<env>/<name>`. The monorepo is tagged `template-v<N>` only after the student repository is fully updated. See [Cutting a release](#cutting-a-release) for version bump behavior, recovery, and the post-release browser check.
