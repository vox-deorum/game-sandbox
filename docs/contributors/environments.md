...
6. Under the hood: the raw, helper-free reference for a reader who outgrows the helpers, holding a raw-array snippet and then the demoted Actions and Observations subsections. Actions gives what integer `act` returns and what every value means (a full table when the action space is small or enumerable, plus the rule for when a returned action is rejected); Observations is a field-by-field table with each field's shape, range, and sentinel values, and a note that values arrive as NumPy arrays.
...
8. Your first improvement: a guided path from the naive starting agent to its first upgrade. Give the hints first, one per paragraph, then the revealed replacement line or lines in a fenced block, taken from a shipped worked example so the page and the example stay the same code. Do not link complete example agents from the page, and do not append further idea lists.
