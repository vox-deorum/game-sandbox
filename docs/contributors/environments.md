...
Students run your environment locally against vanilla PettingZoo, so the import-self-contained modules (`single_agent.py`, `env.py`, `overlay.py`) are copied into the template's `sandbox_env/` by `scripts/generate.py`, and the staleness CI job keeps the copy current. Only the steppable environment is synced — never the harness, recording store, or metadata layer. Run `uv run python scripts/generate.py` after changing any synced module.
