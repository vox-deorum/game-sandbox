...
Rendering must be deterministic: a state produces the same visible frame regardless of what was rendered before it. A replay can therefore jump directly to any tick.
...
`environments/flappy_bird/renderer/` is the reference for a realtime draw-only renderer. `environments/hearts/renderer/` is the reference for turn-based card input and animation.
