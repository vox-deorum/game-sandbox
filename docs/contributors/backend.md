...
| `storage/` | The Kysely schema, the `Storage` interface, the SQLite wiring, migrations, and the `IterationConfig` codec. |
...
Migration 4 (Stage 6.1) is the data foundation the rest of Stage 6 attaches to. It grows the `iterations` row and adds the run/result/rating tables, all still in the one schema declaration. It is Docker-free and is proven by the storage suite on in-memory SQLite.

It first reshapes `iterations`. The submission window, public-play window, and release status are three **independent** gates — a user can reach an iteration when any one is in effect, and each gates a different surface — so they are separate columns, not one overloaded `status`:

- `submission_status` (`open` | `closed`) is the Stage 5 `status`, renamed; its partial unique index (`env_id where submission_status = 'open'`) keeps the one-open-submission invariant.
...
- `label` (operator-facing name), `rating_prompt` (the operator's iteration-wide rater guidance, always editable), and `config` (below) round out the row. The standalone `deps_version` column is **folded into `config`**, so a run's frozen snapshot is the single record of what governed it; the migration copies each row's value into the document before dropping the column.
