...
| `UiDialog` | The modal dialog (focus trap, escape, focus restore, `aria-modal`), wrapping Reka UI Dialog. Its `dismissible` control defaults to true. When false, overlay clicks, Escape, and the header `X` are disabled or hidden for flows that must await an action. A dismissible dialog's header line carries an `X` close button (`aria-label="Close"`) at the far right, so it needs no separate close control unless the control also does something else, such as a `Back` or `Cancel` action. |
| `UiConfirmDialog` | A confirmation flow built on `UiDialog`: a consequence message, explicit confirm, secondary, and cancel actions, the shared error line, and pending state on the action buttons. Cancel closes the dialog by itself. Set `dismissible` to false only for flows that must stay visible while an awaited operation settles, such as session replacement. |
| `UiDialogActions` | The shared right-aligned, wrapping footer for dialog actions. `UiConfirmDialog` renders it for confirmations; use it directly for custom footers in non-confirmation dialogs, with the consequential action first and a ghost Cancel control second. |
...
Confirmation dialogs use `UiConfirmDialog`, which owns the standard footer actions. Keep the action text specific, make irreversible actions `danger`, leave cancellation as a ghost button, and preserve any loading or error state in the feature component. The Cancel action closes the dialog by itself, so handle `@cancel` only when closing needs extra cleanup. Use `UiDialogActions` directly for custom footers in non-confirmation dialogs. Set `dismissible` to false only for workflows that must stay visible until an awaited operation settles, such as session replacement. Existing feature cancellation semantics should otherwise be preserved. A dismissible dialog header carries the universal `X` close button, so do not add a top-right text `Close`; a `Cancel` button stays only where it reads as the paired "no" to a consequential action.
