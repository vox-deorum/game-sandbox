...
`npm run demo` (`scripts/demo.py`) does **not** seed its own data. It snapshots the complete `frontend/e2e/.data/main/` tree into a `demo/` copy and serves that. The database, recordings, submission archives, and LLM telemetry move together, so replay and download sidecars cannot become detached from their database rows. The e2e run is therefore the demo's data builder: better-named, more complete e2e data is a better demo. This is why the leaderboards arcs populate real agents, automated scoreboards, human ratings, model usage, and rating prompts, and why the names are meaningful. The copied database includes the Better Auth accounts the fixtures created, so the demo reaches its personas through a real `/login` sign-in rather than a mocked identity; the bootstrap admin and `ada-lovelace` are real accounts the fixtures create.
...
The second test drives a whole Hearts season, and is what enriches the demo with a populated Hearts Scoreboard. It borrows the Playground's windows (closing them, restoring them in a `finally`), declares the season, and submits Oracle plus three non-LLM `examples/hearts/*` strategies under distinct owners. Because the example folders are diff-only overlays that carry neither their own `manifest.json` nor the `sandbox/` helper package their `agent.py` imports (`from sandbox.cards import …`), the spec stages each agent's `agent.py`, a generated manifest, and that `sandbox/` package (composed the way `scripts/compose.py` does: the base layer, then the Hearts layer overlaid) into a temp folder and submits that, so the agents load exactly like a real submission without running the full compose pipeline. The season enables the local OpenAI-compatible stub's `small` alias. The journey exercises development credentials, runs one match design with two submission seats and two Naive seats, and asserts the released Scoreboard ranks all four agents and the Naive baseline while reporting model usage only for Oracle. Oracle's workflow replay then proves public cost metadata and owner/operator request-body access survive in the demo snapshot. No human ratings are seeded, so the Human Ratings board shows its empty state.

That match is the most expensive shape the suite runs: Hearts sets `seat_order_matters`, so two submission seats over four ready agents expand to twelve ordered seatings plus the appended Naive baseline, thirteen real four-seat container games, several of which compose a multi-submission session image first. The stub upstream is launched locally by `fresh-backend.mjs`, so this coverage uses no external model service or credential. It carries a correspondingly wide `test.setTimeout`. As with the flappy arc, the cheapest lever if CI time becomes a problem is fewer submitted agents.
