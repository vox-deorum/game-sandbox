...
Chargeable time in `act`, `learn`, and `chat` counts toward these limits. `decision_ms` records chargeable `act` time, while the optional hook timings stay separate for display and leaderboard calculation.
