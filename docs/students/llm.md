...
The rate limit is the exception to successful-only accounting. Each authenticated logical request admitted for upstream processing enters the short in-memory rate window whether it succeeds or fails. Backend retry attempts do not create additional rate events.
