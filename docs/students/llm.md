...
Only a call that receives a successful upstream response and completes accounting consumes the durable token budget or creates telemetry. Backend retries remain part of one logical request and create at most one record. An accounting failure returns `meter_unavailable` and blocks more model calls until recovery.

In official sessions, verified proxy time during `act`, `chat`, or `learn`, including retry waits, does not count toward hook, step, or episode timing. Local work still counts. Calls during module import, construction, or `reset` happen before turn timing. See [Agent interface](agent-interface.md#llm-calls).

Rate limits count successful logical request starts. An in-flight request temporarily reserves a slot. Success records one event at the original start time, while failure releases the slot. Backend retries do not add events.
