...
3. **Build**: your code is overlaid onto the pinned dependency set for the season. No new dependencies are installed; everything comes from the template's versioned set.
...
- `template_version`: the dependency-set version your repo targets. It must match the season's version, which is also the `requirements.txt` you installed locally.
...
- **An unknown or mismatched `template_version`**: the deployment must have that version, and it must match the season you are submitting to. If yours is behind, update to the current template release (ask the operator) and resubmit.
