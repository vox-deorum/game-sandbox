...
- It overrides one template file (`agent.py`) with a heuristic agent that ducks under tricks and, when void, dumps its most dangerous card (the queen of spades or a high heart) instead of clinging to it — which clearly outperforms the lowest-legal-card baseline.
