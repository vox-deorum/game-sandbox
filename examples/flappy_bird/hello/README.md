...
Run `uv run python scripts/compose.py flappy_bird hello` to materialize the full runnable agent under `build/examples/flappy_bird/hello/`. This same composed agent, loaded from its manifest, is what the harness CLI plays for the Stage 2 exit criterion.
